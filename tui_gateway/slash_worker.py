"""Persistent slash-command worker — one ChippiCLI per TUI session.
...
def _run(cli: ChippiCLI, command: str) -> str:
...
def main():
    ...
    os.environ["CHIPPI_SESSION_KEY"] = args.session_key
    os.environ["CHIPPI_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ChippiCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
