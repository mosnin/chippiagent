...
# 5. Symlinks the 'chippi' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/chippi-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink chippi into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up chippi command..."

CHIPPI_BIN="$SCRIPT_DIR/venv/bin/chippi"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CHIPPI_BIN" "$COMMAND_LINK_DIR/chippi"
echo -e "${GREEN}✓${NC} Symlinked chippi → $COMMAND_LINK_DISPLAY_DIR/chippi"
...
# ============================================================================
# Seed bundled skills into ~/.chippi/skills/
# ============================================================================

CHIPPI_SKILLS_DIR="${CHIPPI_HOME:-$HOME/.chippi}/skills"
mkdir -p "$CHIPPI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.chippi/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CHIPPI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  chippi status        # Check configuration"
if is_termux; then
    echo "  chippi gateway       # Run gateway in foreground"
else
    echo "  chippi gateway install # Install gateway service (messaging + cron)"
fi
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m chippi_cli.main setup
fi
