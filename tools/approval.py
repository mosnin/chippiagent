...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CHIPPI_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CHIPPI_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CHIPPI_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CHIPPI_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CHIPPI_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CHIPPI_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CHIPPI_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CHIPPI_CRON_SESSION"):
        return False
    if env_var_enabled("CHIPPI_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $CHIPPI_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CHIPPI_ENV_PATH = (
    r'(?:~\/\.chippi/|'
    r'(?:\$home|\$\{home\})/\.chippi/|'
    r'(?:\$chippi_home|\$\{chippi_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bchippi\s+gateway\s+(stop|restart)\b', "stop/restart chippi gateway (kills running agents)"),
    (r'\bchippi\s+update\b', "chippi update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart chippi-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart chippi-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(chippi|gateway|cli\.py)\b', "kill chippi/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill chippi` but not
    # `kill -9 $(pgrep -f chippi)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CHIPPI_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set CHIPPI_INTERACTIVE or CHIPPI_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CHIPPI_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CHIPPI_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CHIPPI_EXEC_ASK")
