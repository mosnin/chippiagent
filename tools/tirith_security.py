...
it is automatically downloaded from GitHub releases to $CHIPPI_HOME/bin/tirith.
...
def _get_chippi_home() -> str:
    """Return the Chippi home directory, respecting CHIPPI_HOME env var."""
    return str(get_chippi_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_chippi_home(), ".tirith-install-failed")
...
def _chippi_bin_dir() -> str:
    """Return $CHIPPI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_chippi_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CHIPPI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_chippi_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CHIPPI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CHIPPI_HOME/bin/tirith
    ...
    chippi_bin = os.path.join(_chippi_bin_dir(), "tirith")
    if os.path.isfile(chippi_bin) and os.access(chippi_bin, os.X_OK):
        _resolved_path = chippi_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return chippi_bin
