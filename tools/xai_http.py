...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``chippi tools`` repaint, tool-registration scans,
    ...
    2. ``~/.chippi/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_chippi_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.chippi/.env`` first, then ``os.environ``.

    Wraps :func:`chippi_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from chippi_cli.config import get_env_value as _chippi_get_env_value

        value = _chippi_get_env_value(name)
...
def chippi_xai_user_agent() -> str:
    """Return a stable Chippi-specific User-Agent for xAI HTTP calls."""
    try:
        from chippi_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Chippi-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Chippi-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``chippi_cli.config.get_env_value`` so keys
    stored in ``~/.chippi/.env`` (the standard Chippi location) are honored —
