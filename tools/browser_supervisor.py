...
One ``CDPSupervisor`` runs per Chippi ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "chippi-dialog-bridge.invalid"
...
(() => {
  if (window.__chippiDialogBridgeInstalled) return;
  window.__chippiDialogBridgeInstalled = true;
  const ENDPOINT = "http://chippi-dialog-bridge.invalid/";
