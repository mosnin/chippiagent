...
selectable via ``chippi tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `chippi tools` → Image Generation."
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`chippi setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `chippi tools` "
        "→ Image Generation (run `chippi plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping chippi with a FAL key configured
