...
# (CHIPPI_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_chippi_home() / "memories"
...
_MEMORY_THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.chippi/\.env|\~/\.chippi/\.env', "chippi_env"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*(?:AGENTS\.md|CLAUDE\.md|\.cursorrules|\.clinerules)', "agent_config_mod"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*\.chippi/(config\.yaml|SOUL\.md)', "chippi_config_mod"),
