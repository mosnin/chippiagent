#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Chippi Skills Hub.
...
Used by chippi_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            chippi_meta = metadata.get("chippi", {})
            if isinstance(chippi_meta, dict):
                tags = chippi_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                chippi_meta = meta_block.get("chippi", {})
                if isinstance(chippi_meta, dict):
                    tags = chippi_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Chippi centralized index source
# ---------------------------------------------------------------------------

CHIPPI_INDEX_URL = "https://chippi-agent.nousresearch.com/docs/api/skills-index.json"
CHIPPI_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "chippi-index.json"
CHIPPI_INDEX_TTL = 6 * 3600  # 6 hours


def _load_chippi_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CHIPPI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if CHIPPI_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - CHIPPI_INDEX_CACHE_FILE.stat().st_mtime
            if age < CHIPPI_INDEX_TTL:
                return json.loads(CHIPPI_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(CHIPPI_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Chippi index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Chippi index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        CHIPPI_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CHIPPI_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ChippiIndexSource(SkillSource):
    """Skill source backed by the centralized Chippi Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_chippi_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "chippi-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "chippi-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ChippiIndexSource(auth=auth), # Centralized index (search + resolved install paths)
