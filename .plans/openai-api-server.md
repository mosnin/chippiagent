# OpenAI-Compatible API Server for Chippi Agent
...
SSE streaming. By exposing this endpoint, chippi-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  chippi-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.chippi/.env`
2. Run `chippi gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with chippi-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns chippi-agent as a model) |
...
Authorization: Bearer chippi-api-key-here
...
But chippi-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "chippi-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured chippi-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of chippi-agent's core prompt
...
Once implemented, chippi-agent works as a drop-in backend for:
