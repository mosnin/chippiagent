#!/usr/bin/env python3
"""Google Workspace API CLI for Chippi Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Chippi-facing JSON contract and falls back to the Python client
...
from _chippi_home import get_chippi_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CHIPPI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Chippi Agent")
