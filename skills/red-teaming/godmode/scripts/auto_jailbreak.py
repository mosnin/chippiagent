...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CHIPPI_HOME", os.path.expanduser("~/.chippi")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CHIPPI_HOME", Path.home() / ".chippi")) / "skills" / "red-teaming" / "godmode"
...
CHIPPI_HOME = Path(os.getenv("CHIPPI_HOME", Path.home() / ".chippi"))
CONFIG_PATH = CHIPPI_HOME / "config.yaml"
PREFILL_PATH = CHIPPI_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Chippi models — already uncensored, just needs clean prompt
    "chippi": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "chippi" in model_lower or "nous" in model_lower:
        return "chippi"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Chippi config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.chippi/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Chippi for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Chippi for changes to take effect.")
