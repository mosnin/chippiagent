...
CHIPPI_HOME_DIR="${CHIPPI_HOME:-$HOME/.chippi}"
CHIPPI_CFG="${CHIPPI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Chippi config has twozero_td MCP entry ──
if [[ ! -f "$CHIPPI_CFG" ]]; then
    echo -e " ${FAIL} Chippi config not found at ${CHIPPI_CFG}"
    manual_steps+=("Create ${CHIPPI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CHIPPI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Chippi config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Chippi config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CHIPPI_CFG} manually"); }
    manual_steps+=("Restart Chippi session to pick up config change")
fi
