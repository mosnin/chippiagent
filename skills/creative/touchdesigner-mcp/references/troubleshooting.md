...
## 5. Chippi Configuration

### Config location

`$CHIPPI_HOME/config.yaml` (defaults to `~/.chippi/config.yaml` when `CHIPPI_HOME` is unset)
...
Restart the Chippi session for changes to take effect. The MCP connection is
