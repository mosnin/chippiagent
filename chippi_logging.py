"""Centralized logging setup for Chippi Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.chippi/logs/`` (profile-aware via ``get_chippi_home()``).
...
from chippi_constants import get_config_path, get_chippi_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_chippi_session_injector", False):
    ...
    _session_record_factory._chippi_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``chippi logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "chippi_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Chippi logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    chippi_home
        Override for the Chippi home directory.  Falls back to
        ``get_chippi_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_chippi_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._chippi_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the chippi group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from chippi_cli.config import is_managed
