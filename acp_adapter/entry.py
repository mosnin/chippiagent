"""CLI entry point for the chippi-agent ACP adapter.

Loads environment variables from ``~/.chippi/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CHIPPI_HOME (default ``~/.chippi``)."""
    from chippi_cli.env_loader import load_chippi_dotenv

    chippi_home = get_chippi_home()
    loaded = load_chippi_dotenv(chippi_home=chippi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", chippi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="chippi-acp",
        description="Run Chippi Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Chippi version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(chippi_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ChippiACPAgent  # noqa: F401

    print("Chippi ACP check OK")


def _run_setup() -> None:
    from chippi_cli.main import main as chippi_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "chippi", "model"]
        chippi_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``chippi postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting chippi-agent ACP adapter")
    ...
    import acp
    from .server import ChippiACPAgent
