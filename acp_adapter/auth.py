"""ACP auth helpers — detect and advertise Chippi authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "chippi-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Chippi runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Chippi can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Chippi.
    ...
    not have Chippi provider credentials configured yet, so Chippi always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Chippi using the currently configured "
