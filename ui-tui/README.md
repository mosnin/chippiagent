...
React + Ink terminal UI for Chippi. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CHIPPI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.chippi/.chippi_history` or under `CHIPPI_HOME`.
...
tui_gateway/
  ...
  slash_worker.py        persistent ChippiCLI subprocess for slash commands
```
