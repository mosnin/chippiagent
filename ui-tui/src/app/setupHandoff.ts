import type { RunExternalProcess } from '@chippi/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`chippi ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching chippi: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`chippi ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
