...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CHIPPI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Chippi on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CHIPPI_TUI_TRUECOLOR=1.
