...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Chippi starts and finds ``~/.openclaw/``.

    Points users at ``chippi claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``chippi claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Chippi, run "
        "`chippi claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `chippi claw cleanup` (renames it to ~/.openclaw.pre-migration — "
