...
Owns one Codex thread per Chippi session. Drives `turn/start`, consumes
...
# Chippi' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CHIPPI_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Chippi/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Chippi session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CHIPPI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CHIPPI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="chippi",
            client_title="Chippi Agent",
            client_version=_get_chippi_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Chippi' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Chippi' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own chippi-tools callback we
            # auto-accept — the user already approved Chippi' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Chippi approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Chippi returns 'once', 'session', 'always', or 'deny'.
...
def _get_chippi_version() -> str:
    """Best-effort Chippi version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("chippi-agent")
