...
Plugins built on Chippi Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Chippi
...
:class:`~chippi_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Chippi' main config
...
handles every provider, fallback chain, and per-task override Chippi
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`chippi_cli.plugins.PluginContext`
