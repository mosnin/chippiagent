...
upstream prefix cache warm.  See ``chippi-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Chippi never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CHIPPI_HOME persona while keeping
    ...
    # Pointer to the chippi-agent skill + docs for user questions about Chippi itself.
    stable_parts.append(CHIPPI_AGENT_HELP_GUIDANCE)
    ...
    # CHIPPI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Chippi profile the agent is running
    # under so it doesn't conflate ~/.chippi/skills/ (default profile) with
    # ~/.chippi/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Chippi profile: default. Other profiles (if any) live "
            "under ~/.chippi/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Chippi profile: {active_profile}. This session reads "
            f"and writes ~/.chippi/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.chippi/skills/, ~/.chippi/plugins/, "
            f"~/.chippi/cron/, ~/.chippi/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the chippi-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Chippi never rebuilds or reinjects parts of it
