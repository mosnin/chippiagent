"""Lightweight internationalization (i18n) for Chippi static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Chippi itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``CHIPPI_LANGUAGE`` environment variable (for tests / quick override)
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a chippi dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`chippi_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("CHIPPI_LANGUAGE")
    if env_lang:
