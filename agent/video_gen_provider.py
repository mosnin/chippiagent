...
as ``kind: backend``) or ``~/.chippi/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``chippi tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CHIPPI_HOME/cache/videos/``, creating parents as needed."""
    from chippi_constants import get_chippi_home

    path = get_chippi_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CHIPPI_HOME/cache/videos/``.
