...
from chippi_constants import get_chippi_home, get_skills_dir, is_wsl
...
_CHIPPI_MD_NAMES = (".chippi.md", "CHIPPI.md")


def _find_chippi_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.chippi.md`` or ``CHIPPI.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Chippi Agent, an intelligent AI assistant created by Nous Research. "
...
CHIPPI_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Chippi Agent "
    "itself, load the `chippi-agent` skill with skill_view(name='chippi-agent') "
    "before answering. Docs: https://chippi-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.chippi/kanban.db`. Your task id is in "
    "`$CHIPPI_KANBAN_TASK`; your workspace is `$CHIPPI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CHIPPI_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `chippi kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Chippi WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Chippi itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Chippi runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Chippi itself is running. The host OS, home, and cwd "
                f"of the Chippi process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Chippi "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_chippi_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.chippi/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("CHIPPI_PLATFORM")
        or get_session_env("CHIPPI_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Chippi Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `chippi-agent` skill "
            "first. It has the actual commands (e.g. `chippi config set …`, `chippi tools`, "
            "`chippi setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: chippi setup, chippi setup tools, chippi setup terminal, chippi status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from CHIPPI_HOME and return its content, or None.
    ...
    """
    try:
        from chippi_cli.config import ensure_chippi_home
        ensure_chippi_home()
    except Exception as e:
        logger.debug("Could not ensure CHIPPI_HOME before loading SOUL.md: %s", e)

    soul_path = get_chippi_home() / "SOUL.md"
...
def _load_chippi_md(cwd_path: Path) -> str:
    """.chippi.md / CHIPPI.md — walk to git root."""
    chippi_md_path = _find_chippi_md(cwd_path)
    if not chippi_md_path:
        return ""
    try:
        content = chippi_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = chippi_md_path.name
        try:
            rel = str(chippi_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".chippi.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", chippi_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .chippi.md / CHIPPI.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CHIPPI_HOME is independent and always included when present.
    ...
    # SOUL.md from CHIPPI_HOME only — skip when already loaded as identity
    if not skip_soul:
