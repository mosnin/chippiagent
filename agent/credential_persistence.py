...
``auth.json``.  They intentionally have no dependency on ``chippi_cli.auth`` so
...
# Sources Chippi owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Chippi-owned OAuth/device-code state)
