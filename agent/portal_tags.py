...
Every Chippi request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Chippi Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=chippi-agent",
        "client=chippi-client-v<__version__>",
    ]

The version is sourced live from ``chippi_cli.__version__`` so it auto-aligns
...
``chippi_cli.__version__`` is the canonical source of truth.
"""
...
def _chippi_version() -> str:
    """Return the current Chippi release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``chippi_cli`` cannot be imported (should
...
def chippi_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=chippi-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=chippi-client-v{_chippi_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=chippi-agent", chippi_client_tag()]
