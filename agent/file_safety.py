...
def _chippi_home_path() -> Path:
    """Resolve the active CHIPPI_HOME (profile-aware) without circular imports."""
    try:
        from chippi_constants import get_chippi_home  # local import to avoid cycles
        return get_chippi_home()
    except Exception:
        return Path(os.path.expanduser("~/.chippi"))


def _chippi_root_path() -> Path:
    """Resolve the Chippi root dir (always the parent of any profile, never per-profile)."""
    try:
        from chippi_constants import get_default_chippi_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    chippi_home = _chippi_home_path()
    chippi_root = _chippi_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(chippi_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(chippi_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved CHIPPI_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("CHIPPI_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Chippi control-plane files: block both the ACTIVE profile's view
    # (chippi_home) AND the global root view. Without the root pass, a
    ...
    chippi_dirs = []
    for base in (_chippi_home_path(), _chippi_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in chippi_dirs:
                chippi_dirs.append(real)
        except Exception:
            continue

    for base_real in chippi_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Chippi path.

    Three categories are blocked:

      * Internal Chippi cache files under ``CHIPPI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CHIPPI_HOME and the global Chippi
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.chippi/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CHIPPI_HOME (profile-aware) AND the global
    # Chippi root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CHIPPI_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    chippi_dirs: list[Path] = []
    for base in (_chippi_home_path(), _chippi_root_path()):
        try:
            real = base.resolve()
            if real not in chippi_dirs:
    ...
    for hd in chippi_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Chippi cache file "
    ...
    for hd in chippi_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Chippi credential store "
    ...
    for hd in chippi_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Chippi MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Chippi MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Chippi profiles are separate CHIPPI_HOME dirs under
...
#
# Reference: May 2026 incident where a chippi-security profile session
# edited skills under both ``~/.chippi/profiles/chippi-security/skills/``
# AND ``~/.chippi/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CHIPPI_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CHIPPI_HOME.
    ...
    """
    try:
        home_real = _chippi_home_path().resolve()
        root_real = _chippi_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Chippi scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Chippi entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Chippi profile {info['target_profile']!r}, but the "
