...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - chippi_home (str): The active CHIPPI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.chippi``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "chippi").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'chippi memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], chippi_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'chippi memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``chippi_home`` is the active CHIPPI_HOME directory path.
