...
Storage
-------
Bundles live in ``~/.chippi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CHIPPI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``chippi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CHIPPI_HOME.

    Honors ``CHIPPI_BUNDLES_DIR`` for tests; falls back to
    ``<CHIPPI_HOME>/skill-bundles``.
    """
    override = os.environ.get("CHIPPI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_chippi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `chippi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
