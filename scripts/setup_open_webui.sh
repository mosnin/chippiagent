...
# Bootstrap Open WebUI against Chippi Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.chippi/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-chippi.sh
...
#   CHIPPI_API_PORT=8642
#   CHIPPI_API_HOST=127.0.0.1
#   CHIPPI_API_MODEL_NAME='Chippi Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Chippi Agent WebUI}"
...
CHIPPI_ENV_FILE="${CHIPPI_ENV_FILE:-$HOME/.chippi/.env}"
CHIPPI_API_PORT="${CHIPPI_API_PORT:-8642}"
CHIPPI_API_HOST="${CHIPPI_API_HOST:-127.0.0.1}"
CHIPPI_API_CONNECT_HOST="${CHIPPI_API_CONNECT_HOST:-127.0.0.1}"
CHIPPI_API_MODEL_NAME="${CHIPPI_API_MODEL_NAME:-Chippi Agent}"
CHIPPI_API_BASE_URL="http://${CHIPPI_API_CONNECT_HOST}:${CHIPPI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-chippi.sh"
LOG_DIR="$HOME/.chippi/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$CHIPPI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$CHIPPI_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.chippi.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.chippi</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.chippi"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.chippi"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-chippi.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Chippi Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-chippi.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.chippi/logs/openwebui.log
StandardError=append:%h/.chippi/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-chippi.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$CHIPPI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Chippi API server is configured...'
  upsert_env API_SERVER_ENABLED true "$CHIPPI_ENV_FILE"
  upsert_env API_SERVER_HOST "$CHIPPI_API_HOST" "$CHIPPI_ENV_FILE"
  upsert_env API_SERVER_PORT "$CHIPPI_API_PORT" "$CHIPPI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$CHIPPI_API_MODEL_NAME" "$CHIPPI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$CHIPPI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Chippi gateway so API server settings take effect...'
  chippi gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${CHIPPI_API_CONNECT_HOST}:${CHIPPI_API_PORT}/health" >/dev/null; then
    log 'Chippi API server did not answer on the first check. Trying to start gateway in the background...'
    nohup chippi gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${CHIPPI_API_CONNECT_HOST}:${CHIPPI_API_PORT}/health" >/dev/null
  ...
  log "Chippi API endpoint: ${CHIPPI_API_BASE_URL}"
