# ============================================================================
# Chippi Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ChippiHome = "$env:LOCALAPPDATA\chippi",
    [string]$InstallDir = "$env:LOCALAPPDATA\chippi\chippi-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/chippi-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/chippi-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Chippi Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ChippiHome)) {
        New-Item -ItemType Directory -Force -Path $ChippiHome | Out-Null
    }
    $envFile = Join-Path $ChippiHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ChippiHome "node"
...
function Install-Git {
    ...
    which Chippi uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\chippi\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Chippi needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Chippi copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\chippi\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CHIPPI_GIT_BASH_PATH`` (User scope) so Chippi can find it in a fresh
    ...
    # Download PortableGit into $ChippiHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ChippiHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Chippi features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Chippi needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CHIPPI_GIT_BASH_PATH`` (User env scope) so Chippi can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ChippiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ChippiHome\git\usr\bin\bash.exe
    $candidates += "$ChippiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ChippiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CHIPPI_GIT_BASH_PATH", $candidate, "User")
            $env:CHIPPI_GIT_BASH_PATH = $candidate
            Write-Info "Set CHIPPI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Chippi may not find Git Bash."
    Write-Info "If needed, set CHIPPI_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ChippiHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ChippiHome\node;$env:Path"
        Write-Success "Node.js $version found (Chippi-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $ChippiHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ChippiHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\chippi-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ChippiHome\node") { Remove-Item -Recurse -Force "$ChippiHome\node" }
                Move-Item $extractedDir.FullName "$ChippiHome\node"
                ...
                $version = & "$ChippiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ChippiHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running chippi processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/chippi-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/chippi-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/chippi-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\chippi-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\chippi-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `chippi.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install chippi-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and chippi.exe broken with
    ...
    # here, not 30 seconds later when the user runs `chippi`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `chippi dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `chippi dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `chippi dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up chippi command..."
    
    if ($NoVenv) {
        $chippiBin = "$InstallDir"
    } else {
        $chippiBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so chippi is globally available
    # On Windows, the chippi.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$chippiBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$chippiBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $chippiBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CHIPPI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\chippi instead
    # of the Unix default ~/.chippi
    $currentChippiHome = [Environment]::GetEnvironmentVariable("CHIPPI_HOME", "User")
    if (-not $currentChippiHome -or $currentChippiHome -ne $ChippiHome) {
        [Environment]::SetEnvironmentVariable("CHIPPI_HOME", $ChippiHome, "User")
        Write-Success "Set CHIPPI_HOME=$ChippiHome"
    }
    $env:CHIPPI_HOME = $ChippiHome
    
    # Update current session
    $env:Path = "$chippiBin;$env:Path"
    
    Write-Success "chippi command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.chippi directory structure
    New-Item -ItemType Directory -Force -Path "$ChippiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ChippiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ChippiHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.chippi/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.chippi/.env"
        }
    } else {
        Write-Info "~/.chippi/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ChippiHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.chippi/config.yaml from template"
        }
    } else {
        Write-Info "~/.chippi/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Chippi's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.chippi/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.chippi/"
    
    # Seed bundled skills into ~/.chippi/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.chippi/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.chippi/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ChippiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.chippi/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'chippi setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\chippi-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\chippi-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\chippi-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.chippi/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'chippi setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m chippi_cli.main setup
    } else {
        python -m chippi_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $chippiCmd = "$InstallDir\venv\Scripts\chippi.exe"
    if (-not (Test-Path $chippiCmd)) {
        $chippiCmd = "chippi"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ChippiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'chippi whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: chippi gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ChippiHome\logs\gateway.log"
            Start-Process -FilePath $chippiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ChippiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: chippi gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: chippi gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ChippiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ChippiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ChippiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ChippiHome\chippi-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   chippi              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   chippi setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   chippi config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   chippi config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   chippi gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   chippi update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Chippi repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Chippi to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/chippi-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
