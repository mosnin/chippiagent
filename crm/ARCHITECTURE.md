...
│   ├── api/
│   │   ├── ai/task/            # On-demand agent streaming endpoint
...
| AI assistant | `app/api/ai/task/route.ts`, `lib/ai-tools/loop.ts`, `lib/ai.ts` | SSE-streamed on-demand agent with a tool-use loop that runs mutating tools in the same turn, sub-agent delegation, RAG context from Supabase pgvector, and `Message.blocks` persistence |
