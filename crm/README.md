...
- **AI agent** — Agent runtime with tool-use over the realtor operating system. Read-only and mutating tools (email, SMS, deal/stage changes, tours) run in the same turn — Chippi sends; it does not wait for per-call approval. Delegates research questions to read-only sub-agents so profile lookups don't bloat the orchestrator's context. See `lib/ai-tools/tools/index.ts` for the tool registry and `lib/ai-tools/skills/*` for the sub-agents.
