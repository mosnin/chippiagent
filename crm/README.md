...
- **AI agent** — Agent runtime with tool-use over the realtor operating system. Read and write tools execute in the same turn: Chippi sends email and SMS, updates deals and stages, and books tours. Rate limits apply per tool. Delegates research questions to read-only sub-agents so profile lookups don't bloat the orchestrator's context. See `lib/ai-tools/tools/index.ts` for the tool registry and `lib/ai-tools/skills/*` for the sub-agents.
