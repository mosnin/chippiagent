---
sidebar_position: 7
title: "Use SOUL.md with Chippi"
description: "How to use SOUL.md to shape Chippi Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Chippi

`SOUL.md` is the **primary identity** for your Chippi instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Chippi to feel like the same assistant every time you talk to it — or if you want to replace the Chippi persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Chippi should be
- what Chippi should avoid stylistically
- how Chippi should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Chippi is and how Chippi speaks
...
Chippi now uses only the global SOUL file for the current instance:

```text
~/.chippi/SOUL.md
```

If you run Chippi with a custom home directory, it becomes:
...
Chippi automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Chippi does not overwrite it
- if the file exists but is empty, Chippi adds nothing from it to the prompt

## How Chippi uses it

When Chippi starts a session, it reads `SOUL.md` from `CHIPPI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Chippi feels.
...
Chippi already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Chippi should sound.

# Avoid
What Chippi should not do.

# Defaults
How Chippi should behave when ambiguity appears.
```
...
Then restart Chippi or start a new session.
...
### I edited SOUL.md but Chippi still sounds the same

Check:
- you edited `~/.chippi/SOUL.md` or `$CHIPPI_HOME/SOUL.md`
...
### Chippi is ignoring parts of my SOUL.md
