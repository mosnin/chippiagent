...
description: "Diagnose and fix common Chippi cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`chippi gateway` for foreground, or `chippi gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `chippi cron tick`.
...
```bash
date
chippi cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.chippi/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.chippi/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.chippi/.env` |
...
| `local` | Write access to `~/.chippi/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `chippi cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `chippi skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `chippi skills list`.
...
2. `~/.chippi/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `chippi cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Chippi config directory). Verify:
```bash
ls ~/.chippi/scripts/your-script.py   # Must exist
chippi cron edit <job_id> --script ~/.chippi/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `chippi skills install <skill-name>`.
...
Jobs are stored in `~/.chippi/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.chippi/cron/jobs.json
chmod 600 ~/.chippi/cron/jobs.json   # Your user should own it
```
...
```bash
chippi cron list                    # Show all jobs, states, next_run times
chippi cron run <job_id>            # Schedule for next tick (for testing)
chippi cron edit <job_id>           # Fix configuration issues
chippi logs                         # View recent Chippi logs
chippi skills list                  # Verify installed skills
```
...
1. Run the job with `chippi cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.chippi/logs/agent.log` for scheduler messages and `~/.chippi/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/chippi-agent](https://github.com/NousResearch/chippi-agent) with:
