---
sidebar_position: 6
title: "Use MCP with Chippi"
description: "A practical guide to connecting MCP servers to Chippi Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Chippi

This guide shows how to actually use MCP with Chippi Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Chippi tool
- you want Chippi to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Chippi to internal APIs, databases, or company systems without modifying Chippi core

Do not use MCP when:
- a built-in Chippi tool already solves the job well
...
- Chippi remains the agent
- MCP servers contribute tools
- Chippi discovers those tools at startup or reload time
...
If you installed Chippi with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Chippi banner/status should show MCP integration when configured
- ask Chippi what tools it has available
...
## WSL2: bridge Chippi in WSL to Windows Chrome

This is the practical setup when:

- Chippi runs inside WSL2
...
In this setup, Chippi does **not** connect to Chrome directly. Instead:

- Chippi runs in WSL
- Chippi starts a local stdio MCP server
...
```text
Chippi (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Chippi stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Chippi core browser transport
...
```bash
chippi mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
chippi mcp test chrome-devtools-win
```

Then start a fresh Chippi session or run:
...
Once loaded, Chippi can use the MCP-prefixed browser tools directly. For example:
...
If Chippi runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Chippi from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Chippi from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Chippi:
...
2. Chippi-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Chippi will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Chippi to reason over a bounded workspace.
...
Now Chippi can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Chippi core.
...
Because Chippi now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
