...
This tutorial walks you through setting up a Telegram bot powered by Chippi Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Chippi Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.chippi/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Chippi itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Chippi Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_chippi_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Chippi Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Chippi Gateway...
...
# View live logs
journalctl --user -u chippi-gateway -f
...
chippi gateway start
chippi gateway stop
tail -f ~/.chippi/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `chippi gateway install` to update the plist.
:::
...
```bash
chippi gateway stop && chippi gateway start
```
...
chippi pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.chippi/.env`:
...
Control how much detail the bot shows when using tools. In `~/.chippi/config.yaml`:
...
Customize how the bot communicates by editing `~/.chippi/SOUL.md`:

For a full guide, see [Use SOUL.md with Chippi](/guides/use-soul-with-chippi).
...
```bash
# From the CLI
chippi cron list          # View all scheduled jobs
chippi cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u chippi-gateway` (Linux) or `~/.chippi/logs/gateway.log` (macOS) |
| Cron job output | `~/.chippi/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.chippi/cron/jobs.json` |
| Pairing data | `~/.chippi/pairing/` |
| Session history | `~/.chippi/sessions/` |
