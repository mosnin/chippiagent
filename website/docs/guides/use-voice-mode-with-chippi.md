---
sidebar_position: 8
title: "Use Voice Mode with Chippi"
description: "A practical guide to setting up and using Chippi voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Chippi
...
- you want Chippi sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Chippi.
...
## Step 1: make sure normal Chippi works first
...
Chippi supports both local and cloud speech stacks.
...
### If you use `chippi setup`

If you choose NeuTTS in the setup wizard, Chippi checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Chippi transcribes and responds
...
- asking Chippi to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Chippi loop.

## Tuning CLI behavior

### Silence threshold

If Chippi starts/stops too aggressively, tune:
...
Chippi stays a normal chat bot, but can speak replies.
...
- you want Chippi to function like a portable research or ops assistant
...
Chippi joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Chippi detects speech boundaries
- transcripts are posted in the associated text channel
- Chippi responds in text and audio
...
1. get text Chippi working
2. install `chippi-agent[voice]`
