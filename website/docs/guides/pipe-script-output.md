...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `chippi send`."
---

# Pipe Script Output to Messaging Platforms

`chippi send` is a small, scriptable CLI that pushes a message to any
messaging platform Chippi is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | chippi send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `chippi
...
chippi send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | chippi send --to telegram:-1001234567890

# Send a file
chippi send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
chippi send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
chippi send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
chippi send --list

# Filter by platform
chippi send --list telegram
```
...
Any platform Chippi ships adapters for works as a target:
...
`chippi send` resolves the message body in this order:

1. **Positional argument** — `chippi send --to telegram "hi"`
2. **`--file PATH`** — `chippi send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | chippi send --to telegram`

When stdin is a TTY (no pipe), Chippi does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  chippi send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `chippi send` reuses your Chippi config, the same script works on
any host where Chippi is installed — no need to export bot tokens into
...
`chippi send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  chippi send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | chippi send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.chippi/bin/chippi send \
...
```bash
./train.py --epochs 200 && \
  chippi send --to telegram "training done" || \
  chippi send --to telegram "training failed (exit $?)"
```
...
chippi send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(chippi send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `chippi send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `chippi send` calls
the platform's REST endpoint directly using credentials from
`~/.chippi/.env` and `~/.chippi/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `chippi gateway start` and retry.
...
# Machine-readable
chippi send --list --json
```

The listing is built from `~/.chippi/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`chippi gateway
...
| Approach | Multi-platform | Reuses Chippi creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `chippi send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`chippi send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `chippi send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `chippi send` shares with cron delivery.
