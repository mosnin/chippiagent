...
```bash
chippi teams-pipeline validate
```
...
```bash
chippi teams-pipeline token-health
chippi teams-pipeline token-health --force-refresh
```
...
```bash
chippi teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
chippi teams-pipeline maintain-subscriptions
chippi teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Chippi cron (recommended if you already run the Chippi gateway)

Chippi ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.chippi/scripts/`. First create the script:

```bash
mkdir -p ~/.chippi/scripts
cat > ~/.chippi/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec chippi teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.chippi/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
chippi cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/chippi-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Chippi Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=chippi
EnvironmentFile=/etc/chippi/env
ExecStart=/usr/local/bin/chippi teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/chippi-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Chippi Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now chippi-teams-pipeline-maintain.timer
systemctl list-timers chippi-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/chippi teams-pipeline maintain-subscriptions >> /var/log/chippi/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.chippi/.env` at the top of a wrapper script that crontab calls.
...
```bash
chippi teams-pipeline subscriptions   # should show expirationDateTime advanced
chippi teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
chippi teams-pipeline list
chippi teams-pipeline list --status failed
chippi teams-pipeline show <job-id>
```

### Replay a stored job

```bash
chippi teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
chippi teams-pipeline fetch --meeting-id <meeting-id>
chippi teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `chippi teams-pipeline maintain-subscriptions --dry-run`
- inspect `chippi teams-pipeline list --status failed`
...
- run `chippi teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `chippi teams-pipeline run`
...
- [ ] `chippi teams-pipeline validate` returns an OK snapshot
- [ ] `chippi teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Chippi cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
