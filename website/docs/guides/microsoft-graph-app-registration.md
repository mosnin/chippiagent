...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.chippi/.env` at the end.
...
4. Fill in:
   - **Name:** `Chippi Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `chippi-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.chippi/.env`:
...
Chippi ships a Graph auth smoke-test. From your Chippi install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.chippi/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `chippi gateway restart`.
