...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Chippi Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Chippi Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Chippi also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `chippi --tui` — see the [TUI](tui.md) guide.
:::
...
chippi chat --model "anthropic/claude-sonnet-4"

# With a specific provider
chippi chat --provider nous        # Use Nous Portal
chippi chat --provider openrouter  # Force OpenRouter

# With specific toolsets
chippi chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
chippi -s chippi-agent-dev,github-auth
chippi chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
chippi --continue             # Resume the most recent CLI session (-c)
chippi --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
chippi chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
chippi -w                         # Interactive mode in worktree
chippi -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Chippi CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Chippi CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CHIPPI_YOLO_MODE` is on (either `chippi --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Chippi to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Chippi supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Chippi replies |
...
```bash
chippi -s chippi-agent-dev,github-auth
chippi chat -s github-pr-workflow -s github-auth
```

Chippi loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.chippi/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.chippi/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Chippi..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Chippi — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Chippi is working, Chippi prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Chippi to the background — just like any terminal process. The shell prints a confirmation:

```
Chippi Agent has been suspended. Run `fg` to bring Chippi Agent back.
```
...
Resume this session with:
  chippi --resume 20260225_143052_a1b2c3
...
```bash
chippi --continue                          # Resume the most recent CLI session
chippi -c                                  # Short form
chippi -c "my project"                     # Resume a named session (latest in lineage)
chippi --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
chippi --resume "refactoring auth"         # Resume by title
chippi -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `chippi sessions rename <id> <title>` from the command line. Use `chippi sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Chippi's SQLite state database under `~/.chippi/state.db`. The database keeps:
...
Chippi immediately confirms the task and gives you back the prompt:
