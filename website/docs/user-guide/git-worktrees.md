...
title: "Git Worktrees"
description: "Run multiple Chippi agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Chippi Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Chippi so each session has a clean, isolated working directory.

## Why Use Worktrees with Chippi?

Chippi treats the **current working directory** as the project root:

- CLI: the directory where you run `chippi` or `chippi chat`
...
git worktree add ../repo-feature feature/chippi-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/chippi-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Chippi there:

```bash
cd ../repo-feature

# Start Chippi in the worktree
chippi
```
...
git worktree add ../repo-experiment-a feature/chippi-a
git worktree add ../repo-experiment-b feature/chippi-b
```
...
- Works on its own branch (`feature/chippi-a` vs `feature/chippi-b`).
...
- Chippi checkpoint data under `~/.chippi/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Chippi experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/chippi-checkpoints-docs`, `feature/chippi-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Chippi from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `chippi -w` (Automatic Worktree Mode)

Chippi has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `chippi/chippi-<hash>`).
...
```bash
chippi -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `chippi -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Chippi session its own clean checkout.
