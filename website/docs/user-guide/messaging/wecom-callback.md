...
Connect Chippi to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Chippi supports two WeCom integration modes:
...
3. Chippi decrypts the message, queues it for the agent
...
(Use `chippi gateway start` only after `chippi gateway install` has registered the systemd/launchd service.)
