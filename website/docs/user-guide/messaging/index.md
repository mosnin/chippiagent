...
description: "Chat with Chippi from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Chippi from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Chippi](/guides/use-voice-mode-with-chippi).
...
```mermaid
flowchart TB
    subgraph Gateway["Chippi Gateway"]
...
```bash
chippi gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
chippi gateway              # Run in foreground
chippi gateway setup        # Configure messaging platforms interactively
chippi gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo chippi gateway install --system   # Linux only: install a boot-time system service
chippi gateway start        # Start the default service
chippi gateway stop         # Stop the default service
chippi gateway status       # Check default service status
chippi gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Chippi Agent to the latest version |
...
Configure per-platform overrides in `~/.chippi/gateway.json`:
...
chippi pairing approve telegram XKGH5N7P

# Other pairing commands:
chippi pairing list          # View pending + approved users
chippi pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Chippi appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.chippi/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.chippi/config.yaml`:
...
```bash
CHIPPI_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
chippi gateway install               # Install as user service
chippi gateway start                 # Start the service
chippi gateway stop                  # Stop the service
chippi gateway status                # Check status
journalctl --user -u chippi-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo chippi gateway install --system
sudo chippi gateway start --system
sudo chippi gateway status --system
journalctl -u chippi-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Chippi will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Chippi installations on the same machine (with different `CHIPPI_HOME` directories), each gets its own systemd service name. The default `~/.chippi` uses `chippi-gateway`; other installations use `chippi-gateway-<hash>`. The `chippi gateway` commands automatically target the correct service for your current `CHIPPI_HOME`.
:::

### macOS (launchd)

```bash
chippi gateway install               # Install as launchd agent
chippi gateway start                 # Start the service
chippi gateway stop                  # Stop the service
chippi gateway status                # Check status
tail -f ~/.chippi/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.chippi.gateway.plist`. It includes three environment variables:
...
- **CHIPPI_HOME** — scopes the gateway to your Chippi installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `chippi gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CHIPPI_HOME` directory gets its own launchd label. The default `~/.chippi` uses `ai.chippi.gateway`; other installations use `ai.chippi.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `chippi-cli` | Full access |
| Telegram | `chippi-telegram` | Full tools including terminal |
| Discord | `chippi-discord` | Full tools including terminal |
| WhatsApp | `chippi-whatsapp` | Full tools including terminal |
| Slack | `chippi-slack` | Full tools including terminal |
| Google Chat | `chippi-google_chat` | Full tools including terminal |
| Signal | `chippi-signal` | Full tools including terminal |
| SMS | `chippi-sms` | Full tools including terminal |
| Email | `chippi-email` | Full tools including terminal |
| Home Assistant | `chippi-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `chippi-mattermost` | Full tools including terminal |
| Matrix | `chippi-matrix` | Full tools including terminal |
| DingTalk | `chippi-dingtalk` | Full tools including terminal |
| Feishu/Lark | `chippi-feishu` | Full tools including terminal |
| WeCom | `chippi-wecom` | Full tools including terminal |
| WeCom Callback | `chippi-wecom-callback` | Full tools including terminal |
| Weixin | `chippi-weixin` | Full tools including terminal |
| BlueBubbles | `chippi-bluebubbles` | Full tools including terminal |
| QQBot | `chippi-qqbot` | Full tools including terminal |
| Yuanbao | `chippi-yuanbao` | Full tools including terminal |
| Microsoft Teams | `chippi-teams` | Full tools including terminal |
| API Server | `chippi-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `chippi-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.chippi/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
