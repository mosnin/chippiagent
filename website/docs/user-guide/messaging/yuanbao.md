...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Chippi Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Chippi to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.chippi/.env`:
...
All standard Chippi commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.chippi/logs/gateway.log`
...
4. Consider enabling verbose logging: `CHIPPI_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Chippi automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
CHIPPI_LOG_LEVEL=debug chippi gateway
```
...
```bash
chippi chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
