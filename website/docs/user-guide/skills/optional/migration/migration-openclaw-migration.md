---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Chippi Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Chippi Agent"
---
...
Migrate a user's OpenClaw customization footprint into Chippi Agent. Imports Chippi-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `chippi skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Chippi Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Chippi`, `Memory`, `Persona`, `Import` |
| Related skills | [`chippi-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-chippi-agent) |
...
# OpenClaw -> Chippi Migration

Use this skill when a user wants to move their OpenClaw setup into Chippi Agent with minimal manual cleanup.
...
```bash
chippi claw migrate              # Full interactive migration
...
chippi claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `chippi setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_chippi.py` to:

- import `SOUL.md` into the Chippi home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Chippi memory entries
- merge OpenClaw command approval patterns into Chippi `command_allowlist`
- migrate Chippi-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.chippi/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Chippi workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.chippi/tts/`
- archive non-secret docs that do not have a direct Chippi destination
...
- `scripts/openclaw_to_chippi.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py`

Do not guess a shorter path like `~/.chippi/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.chippi/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Chippi-compatible secrets, currently:
...
Chippi CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Chippi. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Chippi workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Chippi skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.chippi/skills/migration/openclaw-migration/scripts/openclaw_to_chippi.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Chippi unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Chippi targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Chippi destination. Unsupported auth blobs must still be reported as skipped.
...
- Chippi persona state imported
- Chippi memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.chippi/skills/openclaw-imports/`
