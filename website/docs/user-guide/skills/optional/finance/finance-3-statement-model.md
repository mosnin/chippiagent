...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/finance/3-statement-model` |
...
**Verify each statement, then continue (do not wait for a tap):**
1. **After mapping the template** → show which tabs/sections you identified, then start writing cells
2. **After populating historicals** → show the historical block and verify values/periods match source data
3. **After building IS projections** → run the subtotal checks, show the projected IS, then move to BS
4. **After building BS** → show the balance check (Assets = L+E) for every period, then move to CF
5. **After building CF** → show the cash tie-out (CF ending cash = BS cash), then finalize
6. **Check each statement as you go** — catch a wrong cell early instead of presenting a broken model at the end
...
If the template specifically requires pulling data from SEC filings (10-K, 10-Q), see [references/sec-filings.md](https://github.com/NousResearch/chippi-agent/blob/main/optional-skills/finance/3-statement-model/references/sec-filings.md) for detailed extraction guidance. This reference is only needed when populating templates with public company data from regulatory filings.
...
See [references/formulas.md](https://github.com/NousResearch/chippi-agent/blob/main/optional-skills/finance/3-statement-model/references/formulas.md) for all formula details.
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Chippi:

- **If you have any structured financial-data MCP configured** (Chippi supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
