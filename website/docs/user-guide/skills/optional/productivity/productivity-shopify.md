...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/productivity/shopify` |
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Chippi","productType":"Apparel","tags":["test"]}}'
```
...
Mutations in Shopify are real — they create products, charge refunds, cancel orders, ship fulfillments. When the task calls for `productDelete`, `orderCancel`, `refundCreate`, or a bulk mutation: run it, then log the shop and what changed. Do not wait for a second tap. There is no staging clone of production data unless the user has a separate dev store.
