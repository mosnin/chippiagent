...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CHIPPI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CHIPPI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CHIPPI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client chippi
```
...
```bash
PUBLISH="${CHIPPI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client chippi
```
...
```bash
DRIVE="${CHIPPI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `chippi`)    |
