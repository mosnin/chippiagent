...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Chippi Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.chippi/.env`:
...
```bash
CANVAS="python $CHIPPI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
