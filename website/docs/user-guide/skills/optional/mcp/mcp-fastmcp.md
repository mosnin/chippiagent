...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Chippi or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Chippi. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.chippi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.chippi/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Chippi integration, either:

- configure the server in `~/.chippi/config.yaml` using the `native-mcp` skill, or
...
### Chippi cannot see the deployed server

The server-building part may be correct while the Chippi config is not. Load the `native-mcp` skill and configure the server in `~/.chippi/config.yaml`, then restart Chippi.
