...
description: "Configure and use Honcho memory with Chippi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Chippi -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `chippi skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Chippi

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Chippi profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/chippi#running-honcho-locally-with-chippi

### Verify

```bash
chippi honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Chippi profile's AI peer
...
Honcho models conversations as interactions between **peers**. Chippi creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Chippi instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Chippi run |
| `global` | Single session across all directories |

Manual override: `chippi honcho map my-project-name`
...
Each Chippi profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block chippi.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `chippi.coder` host block in `honcho.json`
...
```bash
chippi honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Chippi when Honcho memory is active.
...
Config file: `$CHIPPI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `chippi honcho setup`. Ensure `memory.provider: honcho` is in `~/.chippi/config.yaml`.

### Memory not persisting across sessions
Check `chippi honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `chippi profile create <name> --clone`. For existing profiles: `chippi honcho sync`.
...
| Command | Description |
|---------|-------------|
| `chippi honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `chippi honcho status` | Show resolved config, connection test, peer info for active profile |
| `chippi honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `chippi honcho disable` | Disable Honcho for the active profile |
| `chippi honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `chippi honcho peers` | Show peer identities across all profiles |
| `chippi honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `chippi honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `chippi honcho sessions` | List known directory-to-session-name mappings |
| `chippi honcho map <name>` | Map current working directory to a Honcho session name |
| `chippi honcho identity` | Seed AI peer identity or show both peer representations |
| `chippi honcho sync` | Create host blocks for all Chippi profiles that don't have one yet |
| `chippi honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Chippi + Honcho |
| `chippi memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `chippi memory status` | Show active memory provider and config |
| `chippi memory off` | Disable external memory provider |
