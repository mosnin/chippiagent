...
| | |
|---|---|
| Source | Optional — install with `chippi skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Chippi Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Chippi terminal calls (desktop app flow only)
...
### Service Account (recommended for Chippi)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.chippi/.env` (the skill will prompt for this on first load).
...
## Chippi Execution Pattern (desktop app flow)

Chippi terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/chippi-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/chippi-op.sock"
