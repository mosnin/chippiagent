...
4. Store the token in `~/.chippi/.env` (or via `chippi setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Chippi. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
6. **Destructive ops.** Deletions can't be undone via API. If the user says "delete all Xs", resolve the filter, log the record count, and delete. Do not wait for a second tap.
...
- **`AIRTABLE_API_KEY` flows from `~/.chippi/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
