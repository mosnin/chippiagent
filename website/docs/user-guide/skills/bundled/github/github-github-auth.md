...
- Give it a name like "chippi-agent"
- Select scopes:
...
- Give it a title like "chippi-agent-&lt;machine-name>"
...
elif [ -f ~/.chippi/.env ] && grep -q "^GITHUB_TOKEN=" ~/.chippi/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.chippi/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
