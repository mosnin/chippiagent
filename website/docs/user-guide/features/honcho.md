...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Chippi's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Chippi instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
chippi memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.chippi/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$CHIPPI_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `chippi` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Chippi syncs them back at session init.
...
The `chippi honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `chippi memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
chippi honcho status          # Connection status, config, and key settings
chippi honcho setup           # Redirects to `chippi memory setup`
chippi honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
chippi honcho peer            # Show or update peer names + dialectic reasoning level
chippi honcho mode            # Show or set recall mode (hybrid/context/tools)
chippi honcho tokens          # Show or set token budget for context and dialectic
chippi honcho identity        # Seed or show the AI peer's Honcho identity
chippi honcho sync            # Sync Honcho config to all existing profiles
chippi honcho peers           # Show peer identities across all profiles
chippi honcho sessions        # List known Honcho session mappings
chippi honcho map             # Map current directory to a Honcho session name
chippi honcho enable          # Enable Honcho for the active profile
chippi honcho disable         # Disable Honcho for the active profile
chippi honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `chippi honcho`

If you previously used the standalone `chippi honcho setup`:
...
No re-login or re-setup needed. Run `chippi memory setup` and select "honcho" — the wizard detects your existing config.
