...
description: "Customize Chippi Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Chippi Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CHIPPI_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Chippi is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Chippi now seeds a default `SOUL.md` automatically in:

```text
~/.chippi/SOUL.md
```

More precisely, it uses the current instance's `CHIPPI_HOME`, so if you run Chippi with a custom home directory, it will use:
...
- Chippi creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Chippi loads `SOUL.md` only from `CHIPPI_HOME`
- Chippi does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Chippi falls back to a built-in default identity
...
If Chippi loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CHIPPI_HOME`, the personality belongs to the Chippi instance itself.

That also makes it easier to teach users:
- "Edit `~/.chippi/SOUL.md` to change Chippi' default personality."
...
- how Chippi should handle uncertainty, disagreement, or ambiguity
...
## What Chippi injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Chippi falls back to a built-in default identity ("You are Chippi Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Chippi ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Chippi, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Chippi its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.chippi/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.chippi/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-chippi)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Chippi speaks
- `display.skin` and `/skin` affect how Chippi looks in the terminal
