...
All skills live in **`~/.chippi/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Chippi at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Chippi to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.chippi/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
chippi chat --toolsets skills -q "What skills do you have?"
chippi chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.chippi/cache/chart-q4-2025.png
...
When a missing value is encountered, Chippi asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `chippi setup` or `~/.chippi/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `chippi config migrate` prompts for unconfigured settings, and `chippi config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.chippi/skills/                  # Single source of truth
...
If you maintain skills outside of Chippi — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Chippi to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.chippi/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.chippi/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Chippi process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Chippi ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.chippi/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.chippi/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `chippi bundles list`.
...
chippi bundles create research

# Overwrite an existing bundle
chippi bundles create backend-dev --skill ... --force

# Delete a bundle
chippi bundles delete backend-dev

# Re-scan ~/.chippi/skill-bundles/ and report changes
chippi bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.chippi/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.chippi/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
chippi skills browse                              # Browse all hub skills (official first)
chippi skills browse --source official            # Browse only official optional skills
chippi skills search kubernetes                   # Search all sources
chippi skills search react --source skills-sh     # Search the skills.sh directory
chippi skills search https://mintlify.com/docs --source well-known
chippi skills inspect openai/skills/k8s           # Preview before installing
chippi skills install openai/skills/k8s           # Install with security scan
chippi skills install official/security/1password
chippi skills install skills-sh/vercel-labs/json-render/json-render-react --force
chippi skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
chippi skills list --source hub                   # List hub-installed skills
chippi skills check                               # Check installed hub skills for upstream updates
chippi skills update                              # Reinstall hub skills with upstream changes when needed
chippi skills audit                               # Re-scan all hub skills for security
chippi skills uninstall k8s                       # Remove a hub skill
chippi skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
chippi skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
chippi skills publish skills/my-skill --to github --repo owner/repo
chippi skills snapshot export setup.json          # Export skill config
chippi skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Chippi. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `chippi skills search <query> --source skills-sh`. Chippi resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Chippi currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Chippi repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Chippi can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
chippi skills search https://mintlify.com/docs --source well-known
chippi skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
chippi skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Chippi can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
chippi skills install openai/skills/k8s
chippi skills tap add myorg/skills-repo
```
...
- Chippi source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Chippi supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Chippi source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Chippi can search and convert agent entries from LobeHub's public catalog into installable Chippi skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Chippi source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Chippi integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Chippi' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Chippi source id: `browse-sh`
- Trust level: `community`

```bash
chippi skills search airbnb --source browse-sh
chippi skills inspect browse-sh/airbnb.com/search-listings-ddgioa
chippi skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Chippi fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Chippi source id: `url`
...
```bash
chippi skills install https://sharethis.chat/SKILL.md
chippi skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
chippi skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `chippi skills update` re-fetches from the same URL automatically when you want to refresh.
...
`chippi skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Chippi | Always trusted |
...
```bash
chippi skills check          # Report which installed hub skills changed upstream
chippi skills update         # Reinstall only the skills with updates available
chippi skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Chippi users add with `chippi skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.chippi.tags`, `version`, `author`, `platforms`, `metadata.chippi.config`).
...
Chippi discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/chippi-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Chippi user can subscribe and install:

```bash
chippi skills tap add my-org/chippi-skills
chippi skills search deploy
chippi skills install my-org/chippi-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.chippi/.hub/taps.json`:
...
The `chippi skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `chippi skills tap list` shows the effective path per tap.
...
```bash
chippi skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.chippi/.hub/taps.json` (created on demand).

## Bundled skill updates (`chippi skills reset`)

Chippi ships with a set of bundled skills in `skills/` inside the repo. On install and on every `chippi update`, a sync pass copies those into `~/.chippi/skills/` and records a manifest at `~/.chippi/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Chippi recomputes the hash of your local copy and compares it to the origin hash:
...
chippi skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `CHIPPI_HOME`, so `chippi -p coder skills reset <name>` only affects that profile.
:::
