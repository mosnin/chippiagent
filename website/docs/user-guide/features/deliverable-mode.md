...
When Chippi Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.chippi/config.yaml` under `agent.custom_instructions`.
...
If you use Chippi' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.chippi/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Chippi Agent's deliverable
