...
Chippi runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Chippi
...
1. Chippi captures a baseline of current diagnostics for the file.
...
…). Chippi auto-detects the binary on PATH or in
`<CHIPPI_HOME>/lsp/bin/`.
...
`node_modules` tree — Chippi installs both packages together when you
run `chippi lsp install typescript` or auto-install fires on first
use.

## CLI

```
chippi lsp status          # service state + per-server install status
chippi lsp list            # registry, optionally --installed-only
chippi lsp install <id>    # eagerly install one server
chippi lsp install-all     # try every server with a known recipe
chippi lsp restart         # tear down running clients
chippi lsp which <id>      # print resolved binary path
```

`chippi lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CHIPPI_HOME>/lsp/bin
...
When `install_strategy: auto`, Chippi installs binaries into
`<CHIPPI_HOME>/lsp/bin/`. NPM packages land in
`<CHIPPI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Chippi-owned and is
...
Servers are kept alive for the life of the Chippi process. There's
...
**`chippi lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CHIPPI_HOME>/lsp/bin/`. Run
`chippi lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `chippi lsp status`**
...
When `chippi lsp status` shows a `Backend warnings` section, install
...
`~/.chippi/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.chippi/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `chippi lsp restart` to clear the set;
