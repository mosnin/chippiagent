...
chat completions — use your Chippi-managed provider subscription as their
...
| Use case | "Use Chippi as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Chippi stores
the refresh token in `~/.chippi/auth.json` — the same place all Chippi
...
Model:      Chippi-4-70B    # or Chippi-4.3-36B, Chippi-4-405B
```
...
If you see `not logged in`, run `chippi login nous`. If you see
...
`chippi_cli/proxy/adapters/<provider>.py` and registering it in
