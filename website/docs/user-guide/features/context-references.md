...
Type `@` followed by a reference to inject content directly into your message. Chippi expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Chippi env: `$CHIPPI_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CHIPPI_HOME/skills/.hub/`
