...
description: "Run Chippi Agent directly on an Android phone with Termux"
---

# Chippi on Android with Termux

This is the tested path for running Chippi Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Chippi from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Chippi now ships a Termux-aware installer path:
...
- links `chippi` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `chippi` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/chippi" "$PREFIX/bin/chippi"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `chippi` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.chippi/.env`.
...
### `chippi doctor` says ripgrep or Node is missing
