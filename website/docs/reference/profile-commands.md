...
This page covers all commands related to [Chippi profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `chippi profile` without a subcommand shows help.
...
```bash
chippi profile use <name>
```

Sets `<name>` as the active profile. All subsequent `chippi` commands (without `-p`) will use this profile.
...
chippi profile use default
```

## `chippi profile create`

```bash
chippi profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `chippi profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `chippi update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
chippi profile create work2 --clone --clone-from work
```

## `chippi profile describe`

```bash
chippi profile describe [<name>] [options]
```
...
chippi profile describe researcher

# Set it explicitly
chippi profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
chippi profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
chippi profile describe --all --auto
```

## `chippi profile delete`

```bash
chippi profile delete <name> [options]
```
...
```bash
chippi profile delete mybot
chippi profile delete mybot --yes
```
...
```bash
chippi profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Chippi home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ chippi profile show work
Profile: work
Path:    ~/.chippi/profiles/work
...
## `chippi profile alias`

```bash
chippi profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Chippi installation.
...
```bash
chippi profile alias work
# Creates/updates ~/.local/bin/work

chippi profile alias work --name mywork
# Creates ~/.local/bin/mywork

chippi profile alias work --remove
# Removes the wrapper script
```

## `chippi profile rename`

```bash
chippi profile rename <old-name> <new-name>
```
...
```bash
chippi profile rename mybot assistant
# ~/.chippi/profiles/mybot → ~/.chippi/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `chippi profile export`

```bash
chippi profile export <name> [options]
```
...
```bash
chippi profile export work
# Creates work.tar.gz in the current directory

chippi profile export work -o ./work-2026-03-29.tar.gz
```

## `chippi profile import`

```bash
chippi profile import <archive> [options]
```
...
```bash
chippi profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

chippi profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`chippi profile export` / `import` are still the right commands for
...
### `chippi profile install`

```bash
chippi profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `chippi -p telemetry`). |
...
chippi profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
chippi profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
chippi profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
chippi profile install ./telemetry/
```

### `chippi profile update`

```bash
chippi profile update <name> [--force-config] [--yes]
```
...
```bash
chippi profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Chippi version, author, env var requirements, the source URL/path, and
...
`chippi profile list` also shows the distribution name and version in a
`Distribution` column, and `chippi profile show <name>` / `delete <name>`
...
chippi profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
chippi profile install https://github.com/your-org/internal-assistant.git
```
...
`chippi_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Chippi version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Chippi can clone from.
3. Tell recipients to run `chippi profile install <your-repo-url>`.
...
## `chippi -p` / `chippi --profile`

```bash
chippi -p <name> <command> [options]
chippi --profile <name> <command> [options]
```

Global flag to run any Chippi command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
chippi -p work chat -q "Check the server status"
chippi --profile dev gateway start
chippi -p personal skills list
chippi -p work config edit
```

## `chippi completion`

```bash
chippi completion <shell>
```
...
- `chippi profile <TAB>` — subcommands (list, use, create, etc.)
- `chippi profile use <TAB>` — profile names
- `chippi -p <TAB>` — profile names
