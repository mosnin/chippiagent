...
[PR #12550](https://github.com/NousResearch/chippi-agent/pull/12550) proposed a
...
(`chippi-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Chippi `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `chippi-acp`, `chippi-api-server`, core toolsets (gated on CDP reachability)
...
- `chippi_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
