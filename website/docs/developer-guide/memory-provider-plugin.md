...
description: "How to build a memory provider plugin for Chippi Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Chippi Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `chippi plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          chippi_home (str): Active CHIPPI_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `chippi memory setup` | **Yes** |
| `save_config(values, chippi_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `chippi memory setup`:
...
All storage paths **must** use the `chippi_home` kwarg from `initialize()`, not hardcoded `~/.chippi`:

```python
# CORRECT — profile-scoped
from chippi_constants import get_chippi_home
data_dir = get_chippi_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.chippi/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `chippi my-provider status`, `chippi my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `chippi <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `chippi --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: chippi my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the chippi my-provider argparse tree.
