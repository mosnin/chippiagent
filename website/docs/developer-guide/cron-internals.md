...
description: "How Chippi stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `chippi_cli/cron.py` | CLI `chippi cron` subcommands |
...
Jobs are stored in `~/.chippi/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `chippi cron` commands are run or during active CLI sessions.
...
```python
# ~/.chippi/scripts/check_competitors.py
...
2. **Environment variable** — `CHIPPI_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.chippi/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `chippi cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `chippi cron` CLI provides direct job management:

```bash
chippi cron list                    # Show all jobs
chippi cron create                  # Interactive job creation (alias: add)
chippi cron edit <job_id>           # Edit job configuration
chippi cron pause <job_id>          # Pause a running job
chippi cron resume <job_id>         # Resume a paused job
chippi cron run <job_id>            # Trigger immediate execution
chippi cron remove <job_id>         # Delete a job
```
