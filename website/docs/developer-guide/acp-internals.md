...
The ACP adapter wraps Chippi' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'chippi-agent[acp]==<version>' chippi-acp`, pointed at the `chippi-agent` PyPI release.
...
- reject options -> Chippi `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Chippi tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Chippi' runtime resolver:

- `acp_adapter/auth.py`
- `chippi_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Chippi provider/credentials. It also always advertises a terminal setup auth method (`chippi-setup`, args `--setup`) so first-run registry clients can open Chippi' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.chippi/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `chippi-acp` toolset definition
- `chippi_cli/main.py` — `chippi acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `chippi-acp` script
