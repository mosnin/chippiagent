...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/chippi-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    chippi_meta = (fm.get("metadata") or {}).get("chippi") or {}
    tags = chippi_meta.get("tags") or []
    related = chippi_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `chippi skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Chippi Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Chippi ships with a large built-in skill library copied into `~/.chippi/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Chippi also syncs bundled skills on `chippi update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.chippi/skills/` tree, it is still shipped with Chippi; restore it with `chippi skills reset <name> --restore`.",
        "",
