...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      chippi-agent = self'.packages.default;
      chippiVenv = chippi-agent.chippiVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "chippi-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${chippiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "chippi-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "chippi-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${chippi-agent}/bin/chippi || (echo "FAIL: chippi binary missing"; exit 1)
          test -x ${chippi-agent}/bin/chippi-agent || (echo "FAIL: chippi-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${chippi-agent}/bin/chippi version 2>&1 | grep -qi "chippi" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "chippi-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in chippi chippi-agent chippi-acp; do
            test -x ${chippi-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "chippi-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking chippi --help ==="
          ${chippi-agent}/bin/chippi --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${chippi-agent}/bin/chippi --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "chippi-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${chippi-agent}/share/chippi-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${chippi-agent}/share/chippi-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CHIPPI_BUNDLED_SKILLS" ${chippi-agent}/bin/chippi || \
            (echo "FAIL: CHIPPI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CHIPPI_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "chippi-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${chippi-agent}/share/chippi-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${chippi-agent}/share/chippi-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CHIPPI_BUNDLED_PLUGINS" ${chippi-agent}/bin/chippi || \
            (echo "FAIL: CHIPPI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CHIPPI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "chippi-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${chippi-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${chippi-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CHIPPI_TUI_DIR" ${chippi-agent}/bin/chippi || \
            (echo "FAIL: CHIPPI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CHIPPI_TUI_DIR set in wrapper"
        ...
        # Verify CHIPPI_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        chippi-node = pkgs.runCommand "chippi-node-version" { } ''
          set -e
          echo "=== Checking CHIPPI_NODE in wrapper ==="
          grep -q "CHIPPI_NODE" ${chippi-agent}/bin/chippi || \
            (echo "FAIL: CHIPPI_NODE not set in wrapper"; exit 1)
          echo "PASS: CHIPPI_NODE present in wrapper"

          CHIPPI_NODE=$(sed -n "s/^export CHIPPI_NODE='\(.*\)'/\1/p" ${chippi-agent}/bin/chippi)
          test -x "$CHIPPI_NODE" || (echo "FAIL: CHIPPI_NODE=$CHIPPI_NODE not executable"; exit 1)
          echo "PASS: CHIPPI_NODE executable at $CHIPPI_NODE"

          NODE_MAJOR=$("$CHIPPI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All CHIPPI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CHIPPI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "chippi-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CHIPPI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CHIPPI_MANAGED guards ==="
          check_blocked "config set" ${chippi-agent}/bin/chippi config set model foo
          check_blocked "config edit" ${chippi-agent}/bin/chippi config edit
        ...
        extra-python-packages = let
          ...
          chippiWithExtra = chippi-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "chippi-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${chippiWithExtra}/bin/chippi || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${chippiWithExtra}/bin/chippi || \
          ...
          if grep -q "PYTHONPATH" ${chippi-agent}/bin/chippi; then
        ...
        extra-dependency-groups = let
          chippiWithGroups = chippi-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "chippi-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${chippiWithGroups}"
          echo "venv: ${chippiWithGroups.chippiVenv}"
        ...
        in pkgs.runCommand "chippi-config-roundtrip" {
          ...
          merge_and_load() {
            local chippi_home="$1"
            export CHIPPI_HOME="$chippi_home"
            ${configMergeScript} ${nixSettings} "$chippi_home/config.yaml"
            ${chippiVenv}/bin/python3 -c '
import json, sys
