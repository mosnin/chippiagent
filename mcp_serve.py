"""
Chippi MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Chippi-specific extra)

Usage:
    chippi mcp serve
    chippi mcp serve --verbose
...
logger = logging.getLogger("chippi.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CHIPPI_HOME."""
    try:
        from chippi_constants import get_chippi_home
        return get_chippi_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CHIPPI_HOME", Path.home() / ".chippi")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from chippi_constants import get_chippi_home
        directory_file = get_chippi_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CHIPPI_HOME", Path.home() / ".chippi")
...
class EventBridge:
    ...
    This is the Chippi equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from chippi_constants import get_chippi_home
            db_file = get_chippi_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CHIPPI_HOME", Path.home() / ".chippi")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Chippi MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "chippi",
        instructions=(
            "Chippi Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Chippi MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
