"""
Email platform adapter for the Chippi gateway.

Allows users to interact with Chippi by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "chippi-agent" "version" "{_chippi_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Chippi Agent")
        ...
        msg_id = f"<chippi-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
