...
There are two ways to add a platform to the Chippi gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.chippi/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Chippi code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `chippi gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `chippi_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Chippi core.
...
And add it to the `chippi-gateway` composite:

```python
"chippi-gateway": {
    "includes": [..., "chippi-your-platform"]
}
```
...
## 12. Status Display (`chippi_cli/status.py`)
...
## 13. Gateway Setup Wizard (`chippi_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ chippi_cli/ toolsets.py \
  --include="*.py" -l | sort -u
