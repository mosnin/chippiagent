...
# Audio file extensions Chippi recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Chippi' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Chippi-created Telegram private-chat
...
from chippi_constants import get_chippi_dir, get_chippi_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.chippi/.env manually."
)
...
# Default location: {CHIPPI_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_chippi_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ChippiAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_chippi_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_chippi_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_chippi_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_chippi_dir("cache/screenshots", "browser_screenshots")
_CHIPPI_HOME = get_chippi_home()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CHIPPI_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CHIPPI_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CHIPPI_MEDIA_TRUST_RECENT_SECONDS"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CHIPPI_HOME / "image_cache",
    _CHIPPI_HOME / "audio_cache",
    _CHIPPI_HOME / "video_cache",
    _CHIPPI_HOME / "document_cache",
    _CHIPPI_HOME / "browser_screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The Chippi home itself contains credentials (auth.json, .env) — only the
    # cache subdirectories under it are explicitly allowlisted above.
    denied.append(_CHIPPI_HOME / ".env")
    denied.append(_CHIPPI_HOME / "auth.json")
    denied.append(_CHIPPI_HOME / "credentials")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under Chippi-managed media caches, or roots the
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?chippi\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+chippi[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("CHIPPI_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "CHIPPI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "CHIPPI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from chippi_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          CHIPPI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          CHIPPI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          CHIPPI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("CHIPPI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("CHIPPI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("CHIPPI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _chippi_run_generation is set on
