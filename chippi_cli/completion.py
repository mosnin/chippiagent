"""Shell completion script generation for chippi CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_chippi_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Chippi Agent bash completion
# Add to ~/.bashrc:
#   eval "$(chippi completion bash)"

_chippi_profiles() {{
    local profiles_dir="$HOME/.chippi/profiles"
...
_chippi_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_chippi_profiles)" -- "$cur"))
        return
    fi
...
complete -F _chippi_completion chippi
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef chippi
# Chippi Agent zsh completion
# Add to ~/.zshrc:
...
_chippi_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.chippi/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.chippi/profiles 2>/dev/null)}}")
    fi
...
_chippi() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_chippi_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'chippi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Chippi Agent fish completion",
        "# Add to your config:",
        "#   chippi completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __chippi_profiles",
        "    echo default",
        "    if test -d $HOME/.chippi/profiles",
        "        ls $HOME/.chippi/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c chippi -f -s p -l profile"
        " -d 'Profile name' -xa '(__chippi_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__chippi_profiles)' -d 'Profile name'"
                )
