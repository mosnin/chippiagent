"""On-demand supply-chain audit for Chippi Agent installs.

Scans three surfaces a Chippi user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Chippi venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.chippi/plugins``
...
def _discover_plugins(chippi_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.chippi/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from chippi_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = chippi_home or Path(get_chippi_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, chippi_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(chippi_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `chippi security audit`."""
    home = Path(get_chippi_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, chippi_home=home
    )
    if total == 0:
