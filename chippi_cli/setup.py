"""
Interactive setup wizard for Chippi Agent.
...
from chippi_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://chippi-agent.nousresearch.com/docs"
...
# display_chippi_home imported lazily at call sites (stale-module safety during chippi update)
...
from chippi_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from chippi_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Chippi Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Chippi using environment variables or config commands:")
    print_info("  chippi config set model.provider custom")
    print_info("  chippi config set model.base_url http://localhost:8080/v1")
    print_info("  chippi config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'chippi setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from chippi_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from chippi_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'chippi setup')")


def _print_setup_summary(config: dict, chippi_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'chippi setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from chippi_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `chippi tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from chippi_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'chippi setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'chippi setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'chippi setup terminal'"))
    ...
    # Spotify (OAuth via chippi auth spotify — check auth.json, not env vars)
    try:
        from chippi_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'chippi setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {chippi_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('chippi setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('chippi setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('chippi setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('chippi setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('chippi setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('chippi config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('chippi config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('chippi config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('chippi', Colors.GREEN)}              Start chatting")
    print(f"   {color('chippi gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('chippi doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `chippi tools` and `chippi setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``chippi model``)
    ...
    provider added to ``chippi model`` is automatically available here.
    ...
    # Delegate to the shared chippi model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from chippi_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: chippi model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from chippi_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Chippi can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Chippi can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'chippi setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``chippi model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from chippi_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.chippi/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Chippi auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via chippi setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'chippi setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Chippi runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'chippi-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'chippi-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CHIPPI_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CHIPPI_MAX_ITERATIONS")
    ...
    print_info("  Run `chippi setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CHIPPI_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Chippi delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `chippi update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://chippi-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CHIPPI_HOME, and print
    ...
    """
    try:
        from chippi_cli.slack_cli import _build_full_manifest
        from chippi_constants import get_chippi_home

        manifest = _build_full_manifest(
            bot_name="Chippi",
            bot_description="Your Chippi agent on Slack",
        )
        target = Path(get_chippi_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `chippi slack manifest --write` anytime to refresh after "
            "Chippi adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Chippi delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Chippi to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from chippi_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://chippi-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://chippi-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  chippi config edit")
    print_info("   Open config in your editor:  chippi config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from chippi_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Chippi from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'chippi setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     chippi config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_chippi_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  chippi gateway")
        else:
            from chippi_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   chippi gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   chippi gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `chippi setup tools` and `chippi tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``chippi_cli.auth`` — lists every supported
    ...
    """
    try:
        from chippi_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from chippi_cli.config
    ...
    if section_key == "model":
        ...
        from chippi_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_chippi.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_chippi migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_chippi", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Chippi.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Chippi to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Chippi at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Chippi at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Chippi at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Chippi at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Chippi equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Chippi config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Chippi.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Chippi's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(chippi_home: Path) -> bool:
    ...
    print_info("Chippi can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: chippi claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=chippi_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: chippi claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Chippi configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Chippi config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Chippi (use chippi claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``chippi setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``chippi setup --portal``)
    that gets a brand-new user from zero to a fully working Chippi session
    ...
    from chippi_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Chippi Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `chippi setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `chippi auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `chippi auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `chippi model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `chippi portal status` to inspect routing.")
    print_info("  Run `chippi` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      chippi setup           — full or quick (auto-detected)
      chippi setup model     — just model/provider
      chippi setup tts       — just text-to-speech
      chippi setup terminal  — just terminal backend
      chippi setup gateway   — just messaging platforms
      chippi setup tools     — just tool configuration
      chippi setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Chippi Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Chippi Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Chippi Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, chippi_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Chippi configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'chippi setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(chippi_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Chippi?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, chippi_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {chippi_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'chippi config edit'")
    ...
    _print_setup_summary(config, chippi_home)


def _run_first_time_quick_setup(config: dict, chippi_home, is_existing: bool):
    ...
    the user can customize later via ``chippi setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'chippi setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    chippi setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  chippi setup gateway")
    print()

    _print_setup_summary(config, chippi_home)


def _run_quick_setup(config: dict, chippi_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'chippi setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Chippi to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'chippi setup gateway'.")
