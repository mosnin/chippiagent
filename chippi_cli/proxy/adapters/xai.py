...
# xAI's public API is OpenAI-compatible for the endpoints Chippi commonly
# uses. The Responses endpoint is included because Chippi' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Chippi-managed OAuth credentials."""

    auth_hint = "chippi auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`chippi auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`chippi auth reset xai-oauth` or re-authenticate with "
                    "`chippi auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `chippi auth add xai-oauth --type oauth`."
            )
