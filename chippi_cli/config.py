"""
Configuration management for Chippi Agent.

Config files are stored in ~/.chippi/ for easy access:
- ~/.chippi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.chippi/.env         - API keys and secrets

This module provides:
- chippi config          - Show current configuration
- chippi config edit     - Open config in editor
- chippi config set      - Set a specific value
- chippi config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.chippi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``chippi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  chippi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `chippi plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "CHIPPI_LANGFUSE_ENV",
    "CHIPPI_LANGFUSE_RELEASE",
    "CHIPPI_LANGFUSE_SAMPLE_RATE",
    "CHIPPI_LANGFUSE_MAX_CHARS",
    "CHIPPI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from chippi_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CHIPPI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_chippi_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Chippi is running in package-manager-managed mode.

    Two signals: the CHIPPI_MANAGED env var (set by the systemd service),
    or a .managed marker file in CHIPPI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade chippi-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Chippi was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.chippi/.install_method`` file (written by installers)
    2. CHIPPI_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_chippi_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.chippi/.install_method."""
    stamp = get_chippi_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade chippi-agent"
    if method == "docker":
        return "docker pull nousresearch/chippi-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade chippi-agent"
        return "pip install --upgrade chippi-agent"
    return "chippi update"
...
def format_managed_message(action: str = "modify this Chippi installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CHIPPI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Chippi installation is managed by NixOS "
            f"(CHIPPI_MANAGED={env_hint}).\n"
            "Edit services.chippi-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Chippi installation is managed by Homebrew "
            f"(CHIPPI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade chippi-agent"
        )

    return (
        f"Cannot {action}: this Chippi installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Chippi."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CHIPPI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, chippi_bin
    or None if container mode is not active, we're already inside the
    container, or CHIPPI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CHIPPI_DEV") == "1":
        return None
    ...
    container_mode_file = get_chippi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "chippi-agent")
    exec_user = info.get("exec_user", "chippi")
    chippi_bin = info.get("chippi_bin", "/data/current-package/bin/chippi")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "chippi_bin": chippi_bin,
    }
...
# Re-export from chippi_constants — canonical definition lives there.
from chippi_constants import get_chippi_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_chippi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_chippi_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the chippi group can
    share state with the gateway service.

    The mode can be overridden via the CHIPPI_HOME_MODE environment variable
    (e.g. CHIPPI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CHIPPI_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CHIPPI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Chippi runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CHIPPI_CONTAINER") or os.environ.get("CHIPPI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CHIPPI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CHIPPI_HOME if the user doesn't have one yet."""
...
def ensure_chippi_home():
    """Ensure ~/.chippi directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_chippi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_chippi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CHIPPI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Chippi-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Chippi auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Chippi sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Chippi
        ...
        # bundled Chippi image, which drops to the `chippi` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Chippi sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Chippi should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``chippi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.chippi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: chippi sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Chippi truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``chippi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``chippi kanban decompose`` and the kanban
        ...
        # ``chippi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `chippi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `chippi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `chippi -c` muscle memory.  Default off so existing
        # users aren't surprised.  CHIPPI_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/chippi`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.chippi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.chippi/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Chippi feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Chippi auto-pauses the goal and
    ...
    # always goes to ~/.chippi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CHIPPI_SKILL_DIR} and ${CHIPPI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `chippi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.chippi/skills/ into
        # ~/.chippi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `chippi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for chippi-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Chippi Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CHIPPI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.chippi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CHIPPI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `chippi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``chippi_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Chippi to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CHIPPI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `chippi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `chippi` launches with
        ...
        # decomposition is manual via `chippi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     chippi-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.chippi/logs/.
    ...
    # update model picker lists without shipping a chippi-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://chippi-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `chippi model` invocation; network failures
    ...
    "gateway": {
        ...
        # uploaded as native gateway attachments. Files inside the Chippi
        # cache (~/.chippi/cache/{documents,images,audio,video,screenshots})
        ...
        # to CHIPPI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        # expanded.
        ...
        # CHIPPI_MEDIA_TRUST_RECENT_FILES.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CHIPPI_MEDIA_TRUST_RECENT_SECONDS.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.chippi/state.db.
    ...
    "sessions": {
        ...
        # default of ``chippi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.chippi/sessions/session_{sid}.json`` on every turn
    ...
    # ``chippi update`` behaviour.
    "updates": {
        # Run a full ``chippi backup``-style zip of CHIPPI_HOME before every
        # ``chippi update``.  Backups land in ``<CHIPPI_HOME>/backups/`` and
        # can be restored with ``chippi import <path>``.  Off by default —
        # on large CHIPPI_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<CHIPPI_HOME>/lsp/bin/`` on first use.
    ...
    # `chippi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.chippi/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.chippi/.env (or your shell) and never in config.yaml.
            ...
            # ~/.chippi/bin/ on first use.  When False you must install
            ...
            # `chippi secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'chippi model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Chippi auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @chippi:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CHIPPI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #chippi)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: chippi-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'chippi-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Chippi API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Chippi API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Chippi API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CHIPPI_TOOL_PROGRESS and CHIPPI_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "CHIPPI_TOOL_PROGRESS": {
    ...
    "CHIPPI_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.chippi.config`` entries, then checks
    ...
    try:
        ...
        # should never break `chippi update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Chippi versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``chippi_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'chippi setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Chippi won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'chippi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CHIPPI_HOME", "~/.chippi")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CHIPPI_TOOL_PROGRESS")
            old_mode = get_env_value("CHIPPI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CHIPPI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CHIPPI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CHIPPI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CHIPPI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CHIPPI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_chippi_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `chippi plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `chippi curator status` has no
    ...
    #   3. Creates `~/.chippi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_chippi_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_chippi_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: chippi config set <key> <value>")
    ...
    # metadata.chippi.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _chippi_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.chippi/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.chippi/config.yaml.
    ...
    (which change ``CHIPPI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — chippi auth) — OpenAI Codex
#   nous         (OAuth — chippi auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.chippi/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.chippi/.env.
    ...
    interactive menu render (`chippi tools`, `chippi setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `chippi tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Chippi env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.chippi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.chippi/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.chippi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.chippi/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Chippi — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Chippi vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.chippi/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Chippi Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  chippi config edit     # Edit config file", Colors.DIM))
    print(color("  chippi config set <key> <value>", Colors.DIM))
    print(color("  chippi setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: chippi config set <key> <value>")
            print()
            print("Examples:")
            print("  chippi config set model anthropic/claude-sonnet-4")
            print("  chippi config set terminal.backend docker")
            print("  chippi config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'chippi config migrate' to add them")
        ...
        print("  chippi config           Show current configuration")
        print("  chippi config edit      Open config in editor")
        print("  chippi config set <key> <value>   Set a config value")
        print("  chippi config check     Check for missing/outdated config")
        print("  chippi config migrate   Update config with new options")
        print("  chippi config path      Show config file path")
        print("  chippi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``chippi config`` UI so users
