"""`chippi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.chippi/checkpoints/``.  Actions:

    chippi checkpoints               # same as `status`
    chippi checkpoints status        # total size, project count, breakdown
    chippi checkpoints list          # per-project checkpoint counts + workdir
    chippi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    chippi checkpoints clear [-f]    # nuke the entire base (asks first)
    chippi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    chippi checkpoints
    chippi checkpoints prune --retention-days 3 --max-size-mb 200
    chippi checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: chippi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``chippi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `chippi checkpoints` → status
