...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or chippi -c.",
    ...
    "Ctrl+Z suspends Chippi to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "chippi -c resumes your most recent CLI session. chippi -c \"project name\" resumes by title.",
    "chippi -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "chippi -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "chippi chat -t web,terminal enables only specific toolsets for a focused session.",
    "chippi chat -s github-pr-workflow preloads a skill at launch.",
    "chippi chat -q \"query\" runs a single non-interactive query and exits.",
    "chippi chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "chippi chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "chippi --yolo bypasses all dangerous command approval prompts for the entire session.",
    "chippi chat --source telegram tags the session for filtering in chippi sessions list.",
    "chippi -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "chippi doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "chippi dump outputs a compact setup summary — great for bug reports.",
    "chippi config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "chippi config edit opens config.yaml in your default editor.",
    "chippi config check scans for missing or stale configuration options.",
    "chippi sessions browse opens an interactive session picker with search.",
    "chippi sessions stats shows session counts by platform and database size.",
    "chippi sessions prune --older-than 30 cleans up old sessions.",
    "chippi skills search react --source skills-sh searches the skills.sh public directory.",
    "chippi skills check scans installed hub skills for upstream updates.",
    "chippi skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "chippi skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "chippi mcp add github --command npx adds MCP servers from the command line.",
    "chippi mcp serve runs Chippi itself as an MCP server for other agents.",
    "chippi auth add lets you add multiple API keys for credential pool rotation.",
    "chippi completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "chippi logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "chippi backup creates a zip backup of your entire Chippi home directory.",
    "chippi profile create coder creates an isolated profile that becomes its own command.",
    "chippi profile create work --clone copies your current config and keys to a new profile.",
    "chippi update syncs new bundled skills to ALL profiles automatically.",
    "chippi gateway install sets up Chippi as a system service (systemd/launchd).",
    "chippi memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "chippi webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: chippi tools disables unused tools, chippi skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "chippi models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as chippi -w).",
    ...
    "CHIPPI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Chippi tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'chippi profile create coder' creates the 'coder' command.",
    "chippi profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Chippi prints a resume command with session ID and stats.",
    "chippi sessions export backup.jsonl exports all sessions for backup or analysis.",
    "chippi -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "chippi skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .chippi/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: chippi cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Chippi runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "chippi gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Chippi via a local macOS server.",
    ...
    "Context files (.chippi.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.chippi/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Chippi loads project context from .chippi.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — chippi tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.chippi/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "chippi chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "chippi plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "chippi acp runs Chippi as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CHIPPI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "chippi login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "chippi status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.chippi/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Chippi your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.chippi/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Chippi auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.chippi/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CHIPPI_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CHIPPI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CHIPPI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.chippi/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'chippi profile create ls' would be rejected.",
    "chippi profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "chippi claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CHIPPI_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via chippi tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.chippi/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.chippi/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Chippi auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Chippi config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Chippi is working.',
    ...
    '/reload reloads ~/.chippi/.env into the running session — pick up new API keys without restarting.',
    ...
    "CHIPPI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.chippi/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.chippi/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'chippi curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "chippi curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'chippi curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'chippi auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'CHIPPI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CHIPPI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'chippi dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'chippi dashboard --tui embeds the full Chippi TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.chippi/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.chippi/dashboard-plugins/ — no npm build required.',
    ...
    'CHIPPI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CHIPPI_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CHIPPI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CHIPPI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.chippi/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.chippi/skills/ so drop-in skills appear without restarting the session.',
    ...
    'chippi -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'chippi chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'chippi chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'chippi chat --ignore-user-config skips ~/.chippi/config.yaml — reproducible bug reports and CI runs.',
    "chippi chat --source tool tags programmatic chats so they don't clutter chippi sessions list.",
    'chippi dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'chippi sessions rename <ID> "new title" renames any past session; chippi sessions delete <ID> removes one.',
    'chippi import restores a session export or profile archive produced by sessions export or profile export.',
    'chippi fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'chippi pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'chippi setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'chippi status --deep runs the full health sweep across every component; plain chippi status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CHIPPI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CHIPPI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.chippi/plugins/ — trust-gated by design.',
    "CHIPPI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CHIPPI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CHIPPI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CHIPPI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CHIPPI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CHIPPI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CHIPPI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CHIPPI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CHIPPI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CHIPPI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CHIPPI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Chippi block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from chippi sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.chippi/dashboard-plugins/.',
]
