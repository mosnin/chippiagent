"""``chippi logs`` — view and filter Chippi log files.
...
Usage examples::
    ...
    chippi logs                    # last 50 lines of agent.log
    chippi logs -f                 # follow agent.log in real time
    chippi logs errors             # last 50 lines of errors.log
    chippi logs gateway -n 100    # last 100 lines of gateway.log
    chippi logs --level WARNING    # only WARNING+ lines
    chippi logs --session abc123   # filter by session ID substring
    chippi logs --component tools  # only tool-related lines
    chippi logs --since 1h         # lines from the last hour
    chippi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_chippi_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Chippi runs — try 'chippi chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_chippi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_chippi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_chippi_home()}/logs/")
        return

    print(f"Log files in {display_chippi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'chippi chat' to generate logs)")
