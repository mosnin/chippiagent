...
on Japanese locales, etc.).  Chippi's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``chippi_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Chippi is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CHIPPI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CHIPPI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Chippi-managed Git install directories so
    ...
    # already-running shells don't see the change — which means chippi
    ...
    # install without Chippi-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Chippi's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Chippi's
    ``chippi config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Chippi (User env var in Windows Settings, or
    export in a PowerShell profile) and Chippi picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Chippi-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\chippi\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``chippi`` from right after install) retain their old PATH.

    Any subprocess Chippi spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Chippi-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "chippi", "git", "cmd"),
        os.path.join(local_appdata, "chippi", "git", "bin"),
        os.path.join(local_appdata, "chippi", "git", "usr", "bin"),
        # Chippi venv Scripts directory — host of the chippi.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes chippi, but harmless to include.
        os.path.join(local_appdata, "chippi", "chippi-agent", "venv", "Scripts"),
