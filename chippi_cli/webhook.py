"""chippi webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.chippi/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _chippi_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: chippi gateway run
"""
...
def webhook_command(args):
    """Entry point for 'chippi webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: chippi webhook {subscribe|list|remove|test}")
        print("Run 'chippi webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (chippi gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: chippi webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from chippi webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (chippi gateway run)")
