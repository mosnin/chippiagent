"""
Unified self-relaunch for Chippi CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``chippi sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``chippi`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from chippi_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched chippi."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_chippi_bin() -> Optional[str]:
    """Find the chippi entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("chippi")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m chippi_cli.main``.
    ...
    ``chippi.exe`` on PATH or the ``sys.executable -m chippi_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with chippi.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_chippi_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "chippi_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh chippi invocation.
    ...
    relaunch contract wants: "run chippi again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``chippi.exe`` (a Python console-script shim that wraps
    ``python -m chippi_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "chippi exited, then
    new chippi started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``chippi`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nChippi relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run chippi.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
