"""Profile distributions — shareable, packaged Chippi profiles via git.

A distribution is a Chippi profile published as a git repository (or
...
* ``chippi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``chippi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``chippi profile``, not a parallel tree):

    chippi profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "chippi-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            chippi_requires=str(data.get("chippi_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.chippi_requires:
            out["chippi_requires"] = self.chippi_requires
        if self.author:
...
def check_chippi_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Chippi {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``chippi profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Chippi profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Chippi distribution."
        )

    # Version check up-front so we fail fast
    check_chippi_requires(manifest.chippi_requires, chippi_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.chippi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="chippi_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `chippi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`chippi profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="chippi_dist_update_") as tmp:
        plan = plan_install(
