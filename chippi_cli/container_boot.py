...
``$CHIPPI_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-chippi-setup
(the stage2 hook) has chowned the volume and seeded $CHIPPI_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CHIPPI_HOME``,
    not under ``profiles/``). The dispatcher in ``chippi_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``chippi gateway start`` (no ``-p``) targets. Without it,
    bare ``chippi gateway start`` inside the container would land on
    ...
    ``$CHIPPI_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        chippi_home: The container's CHIPPI_HOME (typically /opt/data).
            Profiles live under ``<chippi_home>/profiles/<name>/``;
            the default profile lives at ``<chippi_home>`` itself.
    ...
    # named profiles).
    default_prior_state = _read_prior_state(chippi_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(chippi_home)
    ...
    profiles_root = chippi_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `chippi profile create` (config.yaml
            # is not — that comes later via `chippi setup`). Use it as the
            ...
            # via ``chippi -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(chippi_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `chippi -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit chippi
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the chippi user) won't hit EACCES. See
...
def _write_reconcile_log(
    chippi_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CHIPPI_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    chippi_home = Path(os.environ.get("CHIPPI_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        chippi_home=chippi_home, scandir=scandir,
    )
    for a in actions:
        print(
