"""chippi hooks — inspect and manage shell-script hooks.

Usage::

    chippi hooks list
    chippi hooks test <event> [--for-tool X] [--payload-file F]
    chippi hooks revoke <command>
    chippi hooks doctor

Consent records live under ``~/.chippi/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.chippi/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``chippi hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: chippi hooks {list|test|revoke|doctor}")
        print("Run 'chippi hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.chippi/config.yaml.")
        print("See `chippi hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `chippi hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `chippi hooks test` and `chippi hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Chippi wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `chippi hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `chippi hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `chippi hooks doctor`.")
