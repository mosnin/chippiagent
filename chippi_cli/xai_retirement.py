...
Pure logic: walks a Chippi config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `chippi doctor` and a future `chippi migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Chippi config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Chippi config and return retirement issues.
