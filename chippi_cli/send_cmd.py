"""CLI subcommand: ``chippi send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"chippi send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"chippi send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"chippi send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"chippi send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"chippi send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `chippi gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.chippi/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_chippi_env() -> None:
    """Populate ``os.environ`` from ``~/.chippi/.env`` AND bridge top-level
    ...
    startup that ``chippi send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.chippi/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.chippi/config.yaml`` into
       ...
       via ``chippi config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``chippi send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.chippi/.env and ~/.chippi/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "chippi send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  chippi send --to telegram \"hello\"\n"
            "  chippi send --to discord:#ops --file report.md\n"
            "  chippi send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "chippi send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `chippi send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Chippi "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.chippi/.env + ~/.chippi/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  chippi send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  chippi send --list                  # all platforms\n"
            "  chippi send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `chippi send --list telegram`.",
    )
