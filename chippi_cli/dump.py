"""
Dump command for chippi CLI.

Outputs a compact, plain-text summary of the user's Chippi setup
...
from chippi_cli.config import get_chippi_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``chippi dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from chippi_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(chippi_home: Path) -> int:
...
def _cron_summary(chippi_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = chippi_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"chippi_home:      {display_chippi_home()}")
    ...
    toolsets = config.get("toolsets", ["chippi-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(chippi_home)}")
    lines.append(f"  skills:             {_count_skills(chippi_home)}")
