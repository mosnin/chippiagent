...
- ``chippi_cli.providers``        -- canonical provider identity + overlays
- ``chippi_cli.model_normalize``  -- per-provider name formatting
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``chippi config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from chippi_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'chippi model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'chippi doctor' — config issues detected:"
    ...
    # Mirrors the same logic in chippi_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from chippi_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from chippi_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by chippi provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://chippi-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Chippi release. Falls back to the in-repo
    ...
    # --- 1. Check Chippi-mapped providers ---
    for chippi_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(chippi_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CHIPPI_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from chippi_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(chippi_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Chippi release.
        model_ids = curated.get(chippi_id, [])
        if chippi_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(chippi_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Chippi-only providers (nous, openai-codex, copilot, opencode-go) ---
    from chippi_cli.providers import CHIPPI_OVERLAYS
    from chippi_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Chippi provider ID.
    # CHIPPI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Chippi IDs ("copilot").
    _mdev_to_chippi = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CHIPPI_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Chippi slug — e.g. "github-copilot" → "copilot"
        chippi_slug = _mdev_to_chippi.get(pid, pid)
        if chippi_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(chippi_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, chippi_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or chippi_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(chippi_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", chippi_slug, exc)
        ...
        # configured.
        if not has_creds and chippi_slug == "anthropic":
            try:
                ...
                if (chippi_creds and chippi_creds.get("accessToken")) or \
        ...
        if chippi_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(chippi_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(chippi_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(chippi_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Chippi slug, fall back to overlay key
            model_ids = curated.get(chippi_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if chippi_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(chippi_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": chippi_slug,
            "name": get_label(chippi_slug),
            "is_current": chippi_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "chippi",
        ...
        seen_slugs.add(chippi_slug.lower())
        _record_builtin_endpoint(chippi_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CHIPPI_OVERLAYS (keeps /model in sync
    # with `chippi model`).
    try:
        from chippi_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Chippi's canonical write key (matches
            ...
            # Chippi writes ``models:`` as a dict keyed by model id
            # (see chippi_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Chippi's own writer uses; a hyphen variant is accepted
            ...
            # active model. Chippi's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, chippi_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`chippi_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from chippi_cli.models import fetch_openrouter_models
