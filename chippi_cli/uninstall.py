"""
Chippi Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.chippi/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Chippi PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing chippi-agent or chippi PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Chippi Agent" comment and following line
                if '# Chippi Agent' in line or '# chippi-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('chippi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing chippi
                if 'chippi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the chippi wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "chippi",
        Path("/usr/local/bin/chippi"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains chippi_cli reference)
                content = wrapper.read_text()
                if 'chippi_cli' in content or 'chippi-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``chippi gateway run`` processes
    ...
    try:
        from chippi_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from chippi_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CHIPPI_HOME`` and ``CHIPPI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\chippi\git\cmd``, ``%LOCALAPPDATA%\chippi\git\bin``,
#      ``%LOCALAPPDATA%\chippi\git\usr\bin``, ``%LOCALAPPDATA%\chippi\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\chippi\git\`` and Node to
#      ``%LOCALAPPDATA%\chippi\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``chippi dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\chippi\gateway-service\`` and sometimes
...
def _chippi_path_markers(chippi_home: Path) -> list[str]:
    """Path-entry substrings that identify Chippi-owned User-PATH entries."""
    root = str(chippi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare chippi-agent install dir.
    markers = [root + "\\chippi-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CHIPPI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "chippi".  We don't want to catch
    # unrelated entries like "cchippi-foo" or "ephermeral", so we look for
    # backslash-chippi as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(chippi_home: Path) -> list[str]:
    """Strip Chippi-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _chippi_path_markers(chippi_home)
            kept: list[str] = []
...
def remove_chippi_env_vars_windows() -> list[str]:
    """Delete CHIPPI_HOME and CHIPPI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CHIPPI_HOME", "CHIPPI_GIT_BASH_PATH"):
                try:
...
def _is_default_chippi_home(chippi_home: Path) -> bool:
    """Return True when ``chippi_home`` points at the default (non-profile) root."""
    try:
        from chippi_constants import get_default_chippi_root
        return chippi_home.resolve() == get_default_chippi_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CHIPPI_HOME directory.

    We shell out to ``chippi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CHIPPI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m chippi_cli.main` so we don't depend on a `chippi`
    #    wrapper that may be half-removed mid-uninstall.
    chippi_invocation = [_sys.executable, "-m", "chippi_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                chippi_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CHIPPI_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.chippi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.chippi/ for future reinstall
    """
    project_root = get_project_root()
    chippi_home = get_chippi_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CHIPPI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_chippi_home(chippi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Chippi Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {chippi_home / 'config.yaml'}")
    print(f"  Secrets: {chippi_home / '.env'}")
    print(f"  Data:    {chippi_home / 'cron/'}, {chippi_home / 'sessions/'}, {chippi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CHIPPI_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Chippi data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Chippi code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in chippi_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\chippi\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(chippi_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Chippi-owned PATH entries in User environment")

        log_info("Removing CHIPPI_HOME / CHIPPI_GIT_BASH_PATH User env vars...")
        removed_env = remove_chippi_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Chippi-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing chippi command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.chippi/, just remove the chippi-agent subdir
            if chippi_home in project_root.parents or project_root.parent == chippi_home:
    ...
    #     under CHIPPI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(chippi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(chippi_home)
    ...
    # 5. Optionally remove ~/.chippi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CHIPPI_HOME dirs live under
        ...
        try:
            if chippi_home.exists():
                shutil.rmtree(chippi_home)
                log_success(f"Removed {chippi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {chippi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {chippi_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/chippi-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/chippi-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Chippi Agent! ⚕")
    print()
