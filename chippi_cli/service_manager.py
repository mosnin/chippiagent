...
the existing module-level functions in chippi_cli.gateway and
chippi_cli.gateway_windows directly. This protocol is a thin facade
...
``chippi gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``chippi gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged chippi
    ...
    detection always fails. Since every Chippi runtime call inside the
    container drops to chippi via ``s6-setuidgid``, that silent failure
...
# in ``chippi_cli.gateway`` (systemd/launchd) and ``chippi_cli.gateway_windows``
...
# active (set via the ``chippi -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in chippi_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from chippi_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from chippi_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from chippi_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from chippi_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in chippi_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from chippi_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from chippi_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from chippi_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from chippi_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``chippi_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from chippi_cli import gateway_windows
        from chippi_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `chippi profile create`
# runs inside the container (Phase 4). Static services (main-chippi, dashboard)
...
# UID/GID of the in-image ``chippi`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the chippi user.
    ...
    (the ``chippi`` user — UID 10000 — running every Chippi runtime
    ...
    is dispatched as the chippi user.
    ...
    with chippi ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           chippi:chippi, 0755 (parent must already exist)
    ``svc_dir/event/``                     chippi:chippi, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 chippi:chippi, 0755
    ``svc_dir/supervise/event/``           chippi:chippi, 03730
    ``svc_dir/supervise/control``          chippi:chippi, 0660    (FIFO)
    ...
    slot's supervise/ was chippi-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CHIPPI_UID, _CHIPPI_GID)
        except PermissionError:
            # Running as the chippi user already — directory is chippi-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CHIPPI_UID, _CHIPPI_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is chippi-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CHIPPI_UID, _CHIPPI_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``chippi -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`chippi profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-chippi, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CHIPPI_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CHIPPI_HOME=/data/chippi`` is honored at run
             time, not Python-substituted at registration time (OQ8-C).
          2. Activates the bundled venv.
          3. Drops to the chippi user and exec's
             ``chippi -p <profile> gateway run`` (or just ``chippi
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``chippi gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CHIPPI_HOME profile" (the implicit profile that exists at
        the top of $CHIPPI_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``chippi_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CHIPPI_HOME/profiles/default/`` — a
        ...
        ``chippi_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        if profile == "default":
            lines.append("exec s6-setuidgid chippi chippi gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid chippi chippi -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CHIPPI_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CHIPPI_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CHIPPI_HOME=/data/chippi``
        gets its logs under /data/chippi/logs/..., not the build-time
        default.
        """
        import shlex
        ...
        return (
            ...
            f': "${{CHIPPI_HOME:=/opt/data}}"\n'
            f'log_dir="$CHIPPI_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R chippi:chippi "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid chippi s6-log n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with chippi ownership
            ...
            # the chippi user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is chippi-owned (see ``_seed_supervise_skeleton``), and on
