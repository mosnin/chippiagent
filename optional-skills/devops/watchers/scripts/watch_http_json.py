...
Usage (via cron with --no-agent):
    ...
    chippi cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CHIPPI_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Chippi-Watcher/1.0"})
