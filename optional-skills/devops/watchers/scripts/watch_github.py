...
Usage (via cron with --no-agent):
    ...
    chippi cron create chippi-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CHIPPI_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name chippi-issues --repo NousResearch/chippi-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.chippi/.env to avoid the 60 req/hr
