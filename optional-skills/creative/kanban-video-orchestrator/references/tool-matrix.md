...
Maps each role archetype to the Chippi skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public chippi-agent
...
## Chippi skills relevant to video production

### Visual / rendering skills (`chippi-agent/skills/creative/`)
...
### Design / pre-production skills (`chippi-agent/skills/creative/`)
...
### Audio / media skills (`chippi-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`chippi-agent/skills/devops/`)
...
These are **not** Chippi skills but external CLIs / APIs that profiles invoke.
...
## Built-in Chippi tools for media review

These are native Chippi tools — not invoked via terminal but through their own
...
key is present in `~/.chippi/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Chippi profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.chippi/.env` → environment variable.
...
When a new Chippi-public video skill ships:
