...
chippi kanban watch --tenant <project-slug>

# Snapshot of the board
chippi kanban list --tenant <project-slug>
chippi kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
chippi kanban stats --tenant <project-slug>

# Visual dashboard (browser)
chippi dashboard

# Inspect a specific task (includes comments + events)
chippi kanban show <task-id>

# Follow a single task's event stream
chippi kanban tail <task-id>
```

Verify available subcommands with `chippi kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `chippi kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `chippi kanban block <id>` then `chippi kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `chippi kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `chippi kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `chippi profile list` (profile exists?), `chippi status` (gateway/dispatcher up?), and verify tenant. |
...
chippi kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
chippi kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(chippi kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
chippi kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
chippi kanban block <task-id>
chippi kanban archive <task-id>

# Diagnose what happened
chippi kanban show <task-id>      # task body, comments, recent events
chippi kanban tail <task-id>      # follow the live event stream
chippi kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `chippi kanban create` for the director
...
while true; do
    clear
    chippi kanban list --tenant <slug>
    echo "---"
    chippi kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `chippi kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `chippi profile list` and start
