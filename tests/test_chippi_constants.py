"""Tests for chippi_constants module."""
...
class TestGetDefaultChippiRoot:
    """Tests for get_default_chippi_root() — Docker/custom deployment awareness."""

    def test_no_chippi_home_returns_native(self, tmp_path, monkeypatch):
        """When CHIPPI_HOME is not set, returns ~/.chippi."""
        monkeypatch.delenv("CHIPPI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_chippi_root() == tmp_path / ".chippi"

    def test_chippi_home_is_native(self, tmp_path, monkeypatch):
        """When CHIPPI_HOME = ~/.chippi, returns ~/.chippi."""
        native = tmp_path / ".chippi"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(native))
        assert get_default_chippi_root() == native

    def test_chippi_home_is_profile(self, tmp_path, monkeypatch):
        """When CHIPPI_HOME is a profile under ~/.chippi, returns ~/.chippi."""
        native = tmp_path / ".chippi"
    ...
    def test_chippi_home_is_docker(self, tmp_path, monkeypatch):
        """When CHIPPI_HOME points outside ~/.chippi (Docker), returns CHIPPI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(docker_home))
        assert get_default_chippi_root() == docker_home

    def test_chippi_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any CHIPPI_HOME outside ~/.chippi is treated as the root."""
        custom = tmp_path / "my-chippi-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(custom))
        assert get_default_chippi_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CHIPPI_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(profile))
        assert get_default_chippi_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(chippi_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(chippi_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".chippi"
