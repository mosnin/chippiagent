...
binary download are exercised manually by `chippi secrets bitwarden
...
def chippi_home(tmp_path, monkeypatch):
    """Point Chippi at an isolated home directory."""
    home = tmp_path / ".chippi"
    home.mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(home))
    # Some modules cache get_chippi_home; clear if needed.
    import chippi_constants
    if hasattr(chippi_constants, "_CHIPPI_HOME_CACHE"):
        chippi_constants._CHIPPI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(chippi_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(chippi_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(chippi_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from chippi_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under chippi_home/cache/."""
    home = tmp_path / ".chippi"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
