...
def server():
    with patch.dict(
        "sys.modules",
        {
            "chippi_constants": MagicMock(
                get_chippi_home=MagicMock(return_value="/tmp/chippi_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'chippi-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'chippi-release' patched"
    }
