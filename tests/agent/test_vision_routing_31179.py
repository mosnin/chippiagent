...
def isolated_home(monkeypatch):
    """Temp CHIPPI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="chippi_test_31179_")
    chippi_home = os.path.join(test_home, ".chippi")
    os.makedirs(chippi_home)
...
def _fresh_modules():
    """Drop cached chippi modules so each test reloads against current env."""
