...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Chippi passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CHIPPI_HOME", "/users/alice/.chippi/profiles/backend-worker")
        monkeypatch.setenv("CHIPPI_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CHIPPI_KANBAN_DB",
            "/users/alice/.chippi/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.chippi/kanban/boards/smoke"]'
            in cmd
        )
