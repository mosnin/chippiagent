...
def test_chippi_client_tag_includes_current_version():
    """The client tag must reflect chippi_cli.__version__ verbatim."""
    from chippi_cli import __version__
    from agent.portal_tags import chippi_client_tag

    assert chippi_client_tag() == f"client=chippi-client-v{__version__}"


def test_chippi_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import chippi_client_tag

    tag = chippi_client_tag()
    assert tag.startswith("client=chippi-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import chippi_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=chippi-agent" in tags
    assert chippi_client_tag() in tags
    assert len(tags) == 2
