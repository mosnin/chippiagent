...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CHIPPI_HOME under tmp_path."""
    monkeypatch.setenv("CHIPPI_HOME", str(tmp_path / ".chippi"))
    (tmp_path / ".chippi").mkdir()

    # Force the constants/image cache helpers to re-read CHIPPI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("chippi_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_chippi_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CHIPPI_HOME — gateway cleanup
