...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_chippi_env(self):
        """Chippi' own .env inside CHIPPI_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: chippi internal .env is in ~/.chippi/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.chippi/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.chippi/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Chippi cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        chippi_home = tmp_path / ".chippi"
        cache = chippi_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._chippi_home_path", return_value=chippi_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Chippi cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        chippi_home = tmp_path / ".chippi"
        hub = chippi_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_chippi_home(self, tmp_path):
        """The env basename guard does not depend on CHIPPI_HOME resolution."""
        chippi_home = tmp_path / ".chippi"
        chippi_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        chippi_home = tmp_path / ".chippi"
        cache = chippi_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
