...
""",
        ...
        from chippi_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        chippi_home = tmp_path / ".chippi"
        chippi_home.mkdir()
        (chippi_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
        from chippi_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from chippi_cli.plugins import PluginContext, PluginManifest, PluginManager
