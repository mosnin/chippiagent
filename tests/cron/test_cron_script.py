...
- Path resolution (absolute, relative to CHIPPI_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CHIPPI_HOME."""
    chippi_home = tmp_path / ".chippi"
    chippi_home.mkdir()
    (chippi_home / "cron").mkdir()
    (chippi_home / "cron" / "output").mkdir()
    (chippi_home / "scripts").mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CHIPPI_DIR", chippi_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", chippi_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", chippi_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", chippi_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.chippi/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CHIPPI_SESSION_PLATFORM") is None
        assert os.environ.get("CHIPPI_SESSION_CHAT_ID") is None
        assert os.environ.get("CHIPPI_SESSION_CHAT_NAME") is None
