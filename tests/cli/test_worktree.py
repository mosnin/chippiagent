...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("chippi/chippi-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned chippi/* and pr-* branches."""

    def test_prunes_orphaned_chippi_branch(self, git_repo):
        """chippi/chippi-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "chippi/chippi-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "chippi/chippi-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "chippi/chippi-deadbeef" in result.stdout
        ...
        assert "chippi/chippi-deadbeef" in orphaned
        ...
        assert "chippi/chippi-deadbeef" not in result.stdout
