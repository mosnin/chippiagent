...
class TestChippiConstantsFallback:
    """Tests for _chippi_home.py fallback when chippi_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_chippi_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _chippi_home.py with chippi_constants blocked."""
        monkeypatch.setitem(sys.modules, "chippi_constants", None)
        spec = importlib.util.spec_from_file_location("_chippi_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_chippi_home_env_var(self, monkeypatch, tmp_path):
        """When chippi_constants is missing, CHIPPI_HOME comes from env var."""
        monkeypatch.setenv("CHIPPI_HOME", str(tmp_path / "custom-chippi"))
        module = self._load_helper(monkeypatch)
        assert module.get_chippi_home() == tmp_path / "custom-chippi"

    def test_fallback_defaults_to_dot_chippi(self, monkeypatch):
        """When chippi_constants is missing and CHIPPI_HOME unset, default to ~/.chippi."""
        monkeypatch.delenv("CHIPPI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_chippi_home() == Path.home() / ".chippi"

    def test_fallback_ignores_empty_chippi_home(self, monkeypatch):
        """Empty/whitespace CHIPPI_HOME is treated as unset."""
        monkeypatch.setenv("CHIPPI_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_chippi_home() == Path.home() / ".chippi"

    def test_fallback_display_chippi_home_shortens_path(self, monkeypatch):
        """Fallback display_chippi_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("CHIPPI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_chippi_home() == "~/.chippi"

    def test_fallback_display_chippi_home_profile_path(self, monkeypatch):
        """Fallback display_chippi_home() handles profile paths under ~/."""
        monkeypatch.setenv("CHIPPI_HOME", str(Path.home() / ".chippi/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_chippi_home() == "~/.chippi/profiles/coder"

    def test_fallback_display_chippi_home_custom_path(self, monkeypatch):
        """Fallback display_chippi_home() returns full path for non-home locations."""
        monkeypatch.setenv("CHIPPI_HOME", "/opt/chippi-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_chippi_home() == "/opt/chippi-custom"

    def test_delegates_to_chippi_constants_when_available(self):
        """When chippi_constants IS importable, _chippi_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_chippi_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import chippi_constants
        assert module.get_chippi_home is chippi_constants.get_chippi_home
        assert module.display_chippi_home is chippi_constants.display_chippi_home
