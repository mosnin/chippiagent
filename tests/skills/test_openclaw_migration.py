...
"""
    ...
    assert "Tyler Williams > Active Projects: Chippi Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `chippi cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # chippi_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.chippi/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "chippi_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Chippi``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Chippi prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Chippi to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Chippi config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Chippi uses tools well"
    # All-lowercase matches → lowercase ``chippi``; this preserves the
    # real filesystem path ``~/.chippi`` (Chippi home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "chippi should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Chippi remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "chippi prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Chippi was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "chippi uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Chippi said to ask Chippi about Chippi settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.chippi`` (the real Chippi home), not the broken
    ``.Chippi``.
    ...
    rewritten to ``~/.Chippi/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.chippi/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .chippi directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.chippi'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "chippi config path: ~/.chippi/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Chippi" in result
