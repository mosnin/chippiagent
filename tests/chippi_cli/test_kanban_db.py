"""Tests for the Kanban DB layer (chippi_cli.kanban_db)."""
...
from chippi_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated CHIPPI_HOME with an empty kanban DB."""
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(home))
    monkeypatch.delenv("CHIPPI_KANBAN_DB", raising=False)
    monkeypatch.delenv("CHIPPI_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("CHIPPI_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import chippi_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from chippi_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Chippi profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="chippi-task", assignee="daily")
...
def test_scratch_workspace_created_under_chippi_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Chippi-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``CHIPPI_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("CHIPPI_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Chippi' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Chippi' DB/metadata/logs on task completion.
    """
...
# spawned with `chippi -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's CHIPPI_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's CHIPPI_HOME."""

    def _set_home(self, monkeypatch, tmp_path, chippi_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
        monkeypatch.delenv("CHIPPI_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_chippi(
        self, tmp_path, monkeypatch
    ):
        # Standard install: CHIPPI_HOME == ~/.chippi, no profile active.
        default_home = tmp_path / ".chippi"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.chippi/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.chippi/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.chippi/kanban.db.
        default_home = tmp_path / ".chippi"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # CHIPPI_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `chippi -p coder`).
    ...
    def test_docker_custom_chippi_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: CHIPPI_HOME points outside ~/.chippi.
        # `get_default_chippi_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".chippi"`.
        custom_root = tmp_path / "opt" / "chippi"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: CHIPPI_HOME=/opt/chippi/profiles/coder;
        # `get_default_chippi_root()` walks up to /opt/chippi because
    ...
    def test_explicit_override_via_chippi_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: CHIPPI_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".chippi"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(profile_home))
        monkeypatch.setenv("CHIPPI_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(default_home))
        monkeypatch.setenv("CHIPPI_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # CHIPPI_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile CHIPPI_HOME and reads.
        monkeypatch.setenv("CHIPPI_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_chippi_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # CHIPPI_KANBAN_DB pins the file path directly and beats both
        # CHIPPI_KANBAN_HOME and the `get_default_chippi_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(default_home))
        monkeypatch.setenv("CHIPPI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("CHIPPI_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows CHIPPI_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_chippi_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # CHIPPI_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".chippi"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("CHIPPI_HOME", str(default_home))
        monkeypatch.setenv("CHIPPI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("CHIPPI_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows CHIPPI_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CHIPPI_HOME", str(default_home))
        monkeypatch.setenv("CHIPPI_KANBAN_DB", "   ")
        monkeypatch.setenv("CHIPPI_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject CHIPPI_KANBAN_DB
        # and CHIPPI_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites CHIPPI_HOME.
        default_home = tmp_path / ".chippi"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["CHIPPI_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["CHIPPI_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["CHIPPI_KANBAN_TASK"] == "t_dispatch_env"
        assert env["CHIPPI_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see chippi_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("chippi_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="chippi_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'chippi kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_chippi_argv()
#
# Workers spawned by the dispatcher must use a `chippi` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["chippi", ...]`
...
def test_resolve_chippi_argv_prefers_path_shim(monkeypatch):
    """When `chippi` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import chippi_cli.kanban_db as kb

    monkeypatch.delenv("CHIPPI_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/chippi")
    argv = kb._resolve_chippi_argv()
    assert argv == ["/usr/local/bin/chippi"]


def test_resolve_chippi_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import chippi_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("CHIPPI_BIN", ".\\chippi.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_chippi_argv() == [os.path.abspath(".\\chippi.exe")]


def test_resolve_chippi_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "chippi.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("CHIPPI_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_chippi_argv() == [sys.executable, "-m", "chippi_cli.main"]


def test_resolve_chippi_argv_honors_chippi_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like CHIPPI_BIN lets service managers pin the executable."""
    import shutil
    import chippi_cli.kanban_db as kb

    shim = tmp_path / "bin" / "chippi"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("CHIPPI_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_chippi_argv() == [str(shim)]


def test_resolve_chippi_argv_chippi_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare CHIPPI_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import chippi_cli.kanban_db as kb

    cwd_chippi = tmp_path / "chippi"
    cwd_chippi.write_text("wrong\n", encoding="utf-8")
    cwd_chippi.chmod(cwd_chippi.stat().st_mode | stat.S_IXUSR)
    path_chippi = tmp_path / "bin" / "chippi"
    path_chippi.parent.mkdir()
    path_chippi.write_text("right\n", encoding="utf-8")
    path_chippi.chmod(path_chippi.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_chippi.parent))
    monkeypatch.setenv("CHIPPI_BIN", "chippi")

    assert kb._resolve_chippi_argv() == [str(path_chippi)]


def test_resolve_chippi_argv_chippi_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare CHIPPI_BIN does not accept current-directory shadow executables."""
    import sys
    import chippi_cli.kanban_db as kb

    (tmp_path / "chippi.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_chippi_argv_chippi_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved CHIPPI_BIN batch shim is not used as worker argv[0]."""
    import sys
    import chippi_cli.kanban_db as kb
...
def test_resolve_chippi_argv_chippi_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved CHIPPI_BIN command names do not delegate cwd search to Popen."""
    import sys
    import chippi_cli.kanban_db as kb
...
def test_resolve_chippi_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m chippi_cli.main`.

    Pins the correct module name (NOT `chippi` — there is no top-level
    `chippi` package). Regression for #23198: the original PR shipped
    `python -m chippi` which fails with `No module named chippi` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_chippi_argv()
    assert argv == [sys.executable, "-m", "chippi_cli.main"]


def test_resolve_chippi_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `chippi_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m chippi_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("CHIPPI_BIN", None)
    ...
    assert "Chippi Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="chippi_cli.kanban_db"):
        with kb.connect() as conn:
