"""Tests for chippi_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("chippi_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from chippi_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("chippi_cli.config.load_config")
    ...
    @patch("chippi_cli.config.load_config")
    @patch.dict("os.environ", {"CHIPPI_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CHIPPI_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """CHIPPI_SESSION_PLATFORM should be used when CHIPPI_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_chippi_platform_takes_precedence(self, tmp_path, monkeypatch):
        """CHIPPI_PLATFORM should win over CHIPPI_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("CHIPPI_HOME", str(tmp_path))
        monkeypatch.setenv("CHIPPI_PLATFORM", "telegram")
        monkeypatch.setenv("CHIPPI_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CHIPPI_HOME", str(tmp_path))
        monkeypatch.setenv("CHIPPI_PLATFORM", "telegram")
        monkeypatch.setenv("CHIPPI_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from chippi_cli.skills_config import _get_categories
        skills = [
