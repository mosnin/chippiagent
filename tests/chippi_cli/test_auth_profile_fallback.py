...
When ``CHIPPI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.chippi/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.chippi            (has its own auth.json fixture)
    * Profile     -> tmp_path/.chippi/profiles/coder   (active, CHIPPI_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CHIPPI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.chippi as the "real home" — which is NOT equal
    # to the CHIPPI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    chippi_home = tmp_path / "classic"
    chippi_home.mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))

    _write(chippi_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from chippi_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CHIPPI_HOME is set to a custom path that is NOT under
    # ~/.chippi/profiles/ — get_default_chippi_root() returns CHIPPI_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from chippi_cli.auth import read_credential_pool, write_credential_pool
