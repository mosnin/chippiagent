"""Tests for the Phase 4 s6 dispatch helper in chippi_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`chippi gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from chippi_cli import gateway as gw
    monkeypatch.setattr(
        "chippi_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "chippi_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "chippi_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "chippi_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`chippi -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from chippi_cli import gateway as gw
    from chippi_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "chippi_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "chippi profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    from chippi_cli import gateway as gw
    from chippi_cli.service_manager import S6CommandError
    ...
    monkeypatch.setattr(
        "chippi_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
