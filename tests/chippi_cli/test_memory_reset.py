"""Tests for the `chippi memory reset` CLI command.
...
- Profile-scoped reset (uses CHIPPI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CHIPPI_HOME with memory files."""
    chippi_home = tmp_path / ".chippi"
    memories = chippi_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nChippi repo is at ~/.chippi/chippi-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `chippi memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        chippi_home = tmp_path / ".chippi"
        (chippi_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CHIPPI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        chippi_home = tmp_path / ".chippi"
        chippi_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))

        # The memories dir won't exist; get_chippi_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
