...
``chippi setup`` / ``chippi model`` when an API key already exists in ``.env``.
...
def _run_prompt(existing_key, choice, new_key="", provider_id="", pconfig_name="deepseek"):
    ...
    from chippi_cli import main as m

    pconfig = _pconfig(pconfig_name)
    with patch("builtins.input", return_value=choice), \
         patch("chippi_cli.secret_prompt.masked_secret_prompt", return_value=new_key):
