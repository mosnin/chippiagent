...
subparser, placing the flag BEFORE the subcommand (e.g. 'chippi --yolo chat')
...
def _build_parser():
    """Build the chippi argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("chippi_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "chippi_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets CHIPPI_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        args = parser.parse_args(["chat"])
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("CHIPPI_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `chippi gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `chippi <argv>` must exit 0 (help) rather than
