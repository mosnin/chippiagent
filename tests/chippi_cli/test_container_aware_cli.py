...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CHIPPI_HOME with .container-mode file."""
    chippi_home = tmp_path / ".chippi"
    chippi_home.mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
    monkeypatch.delenv("CHIPPI_DEV", raising=False)

    container_mode = chippi_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=chippi-agent\n"
        "exec_user=chippi\n"
        "chippi_bin=/data/current-package/bin/chippi\n"
    )
    return chippi_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("chippi_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "chippi-agent"
    assert info["exec_user"] == "chippi"
    assert info["chippi_bin"] == "/data/current-package/bin/chippi"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("chippi_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_chippi_dev(container_env, monkeypatch):
    """Returns None when CHIPPI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("CHIPPI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_chippi_dev_zero(container_env, monkeypatch):
    """CHIPPI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("CHIPPI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        chippi_home = Path(tmpdir) / ".chippi"
        chippi_home.mkdir()
        (chippi_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("chippi_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_chippi_home": lambda: chippi_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CHIPPI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=chippi-custom\n"
        "exec_user=myuser\n"
        "chippi_bin=/opt/chippi/bin/chippi\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "chippi-custom"
    assert info["exec_user"] == "myuser"
    assert info["chippi_bin"] == "/opt/chippi/bin/chippi"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from chippi_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "chippi"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "chippi-agent" in cmd
    assert "/data/current-package/bin/chippi" in cmd
    assert "chat" in cmd
