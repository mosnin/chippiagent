"""Tests for issue #26670 — concurrent chippi.exe detection and improved
quarantine retry / reboot-deferred fallback during `chippi update` on Windows.
...
from chippi_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_chippi_instances
# helper (and need the autouse stub in tests/chippi_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_chippi_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "chippi.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "chippi.exe").write_bytes(b"")
    (scripts_dir / "chippi-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_chippi_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "chippi.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_chippi_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "chippi.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "chippi.exe"),
    ...
    assert result == [(other_pid, "chippi.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning chippi.EXE (uppercase ext) from same path
    upper = str(shim).replace("chippi.exe", "CHIPPI.EXE")
    procs = [_make_proc(9999, upper, "CHIPPI.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_chippi_instances(scripts_dir)

    assert result == [(9999, "CHIPPI.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_chippi_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: chippi.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "chippi.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_chippi(_winp, tmp_path):
    """A sibling chippi.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "chippi.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED chippi.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "chippi.exe"),
        _make_proc(sibling_pid, str(shim), "chippi.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "chippi.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "chippi.exe"),
        _make_proc(grandparent_pid, str(shim), "chippi.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "chippi.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "chippi.exe"),
        _make_proc(other_pid, str(shim), "chippi.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "chippi.exe"), (5678, "chippi.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "chippi.exe" in msg
    assert "Chippi Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "chippi.exe") in msg


# ---------------------------------------------------------------------------
# _quarantine_running_chippi_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "chippi.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_chippi_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("chippi.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_chippi_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Chippi Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another chippi.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "chippi.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_chippi_instances", detect
    ), patch.object(
