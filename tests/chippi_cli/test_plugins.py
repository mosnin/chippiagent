"""Tests for the Chippi plugin system (chippi_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<chippi_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<chippi_home>/plugins/``; we derive
    ``<chippi_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <CHIPPI_HOME>/config.yaml.
        # Config is always read from CHIPPI_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        chippi_home_str = os.environ.get("CHIPPI_HOME")
        if chippi_home_str:
            chippi_home = Path(chippi_home_str)
        else:
            chippi_home = base.parent
        chippi_home.mkdir(parents=True, exist_ok=True)
        cfg_path = chippi_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.chippi/plugins/ are discovered."""
        plugins_dir = tmp_path / "chippi_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("CHIPPI_HOME", str(tmp_path / "chippi_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CHIPPI_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".chippi" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        chippi_home = tmp_path / "chippi_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under chippi_plugins.<name>."""
        plugins_dir = tmp_path / "chippi_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("CHIPPI_HOME", str(tmp_path / "chippi_test"))

        # Clean up any prior namespace module
        sys.modules.pop("chippi_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "chippi_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="chippi_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "chippi_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import chippi_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("chippi_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        chippi_home = tmp_path / "chippi_test"
        plugins_dir = chippi_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "chippi_test" / "plugins"
        # Set CHIPPI_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("chippi_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("chippi_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("chippi_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("chippi_cli.plugins.PluginContext.dispatch_tool.__module__", "chippi_cli.plugins"):
...
class TestPluginDebugLogging:
    """CHIPPI_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("CHIPPI_PLUGINS_DEBUG", raising=False)
        from chippi_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With CHIPPI_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("CHIPPI_PLUGINS_DEBUG", "1")
        from chippi_cli import plugins as plugins_mod
