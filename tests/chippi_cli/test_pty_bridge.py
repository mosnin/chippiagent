"""Unit tests for chippi_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf chippi-ok"])
        try:
            output = _read_until(bridge, b"chippi-ok")
            assert b"chippi-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$CHIPPI_PTY_TEST\""],
            env={**os.environ, "CHIPPI_PTY_TEST": "pty-env-works"},
        )
        try:
