"""Tests for IRC gateway configuration via `chippi setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Chippi install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Chippi install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#chippi")
            monkeypatch.setenv("IRC_NICKNAME", "chippi-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `chippi setup gateway` experience with IRC present."""
