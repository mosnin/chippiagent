...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("chippi_cli.config.get_managed_system", return_value=None), \
         patch("chippi_cli.config.get_chippi_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When CHIPPI_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("chippi_cli.config.get_managed_system", return_value="NixOS"), \
         patch("chippi_cli.config.get_chippi_home", return_value=tmp_path):
        from chippi_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from chippi_cli.config import recommended_update_command_for_method
...
def test_docker_detected_via_dockerenv(tmp_path):
    with patch("chippi_cli.config.get_managed_system", return_value=None), \
         patch("chippi_cli.config.get_chippi_home", return_value=tmp_path), \
         patch("chippi_constants.is_container", return_value=True):
        from chippi_cli.config import detect_install_method
        assert detect_install_method(project_root=tmp_path) == "docker"
