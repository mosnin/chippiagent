"""Behavior tests for chippi_cli.inventory.
...
from chippi_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("chippi_cli.models.provider_model_ids") as mock_pm:
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # chippi model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `chippi model`")
        )
