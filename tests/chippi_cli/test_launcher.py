"""Tests for the top-level `./chippi` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./chippi` should use `chippi_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "chippi"
    called = []

    fake_main_module = types.ModuleType("chippi_cli.main")

    def fake_main():
        called.append("chippi_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "chippi_cli.main", fake_main_module)
