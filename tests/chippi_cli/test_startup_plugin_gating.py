...
``chippi_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``chippi --help``, ``chippi version``,
``chippi logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``chippi --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from chippi_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["chippi", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["chippi", "-w", "--tui"], None),
        (["chippi", "version"], "version"),
        (["chippi", "--tui", "chat"], "chat"),
        (["chippi", "-w", "logs"], "logs"),
        (["chippi", "chat", "hello world"], "chat"),
        (["chippi", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["chippi", "-m", "gpt5", "chat"], "chat"),
        (["chippi", "--model", "gpt5", "chat", "hi"], "chat"),
        (["chippi", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["chippi", "-z", "hello world"], None),
        (["chippi", "-z", "hello", "chat"], "chat"),
        (["chippi", "--model=gpt5", "chat"], "chat"),     # inline form
        (["chippi", "--", "chat"], "chat"),               # -- terminator
        (["chippi", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["chippi", "some-plugin-cmd"], "some-plugin-cmd"),
        (["chippi", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["chippi"],                          # bare → chat
        ["chippi", "--help"],                # top-level help
        ["chippi", "-h"],
        ["chippi", "version"],               # known built-in
        ["chippi", "logs"],
        ["chippi", "gateway", "run"],
        ["chippi", "--tui"],
        ["chippi", "-w", "--tui"],
        ["chippi", "chat", "hi"],
        ["chippi", "help"],                  # accepted built-in-ish
        ["chippi", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["chippi", "meet", "join"],          # potential google_meet plugin
        ["chippi", "honcho", "status"],      # potential memory plugin
        ["chippi", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``chippi help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"chippi_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
