"""Tests for Codex auth — tokens stored in Chippi auth store (~/.chippi/auth.json)."""
...
def _setup_chippi_auth(chippi_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Chippi auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_chippi_auth(chippi_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_chippi_auth(chippi_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
    ...
    monkeypatch.setattr("chippi_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    chippi_home = tmp_path / "chippi"
    _setup_chippi_auth(chippi_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Chippi auth store, not ~/.codex/."""
    chippi_home = tmp_path / "chippi"
    codex_home = tmp_path / "codex-cli"
    chippi_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "chippi-at", "refresh_token": "chippi-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Chippi store
    assert not (codex_home / "auth.json").exists()

    # Chippi auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "chippi-at"


def test_resolve_returns_chippi_auth_store_source(tmp_path, monkeypatch):
    chippi_home = tmp_path / "chippi"
    _setup_chippi_auth(chippi_home)
    monkeypatch.setenv("CHIPPI_HOME", str(chippi_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "chippi_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("chippi_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("chippi_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
