"""Tests for ``chippi debug`` CLI command and debug utilities."""
...
@pytest.fixture
def chippi_home(tmp_path, monkeypatch):
    """Set up an isolated CHIPPI_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from chippi_cli.debug import _upload_paste_rs
        ...
        with patch("chippi_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "chippi_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from chippi_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from chippi_cli.debug import upload_to_pastebin

        with patch("chippi_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("chippi_cli.debug._upload_paste_rs",
             ...
             patch("chippi_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, chippi_home):
        from chippi_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, chippi_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (chippi_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, chippi_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = chippi_home / "logs" / "agent.log"
        from chippi_cli import debug
    ...
    def test_truncates_large_file(self, chippi_home):
        """Files larger than max_bytes get tail-truncated."""
        from chippi_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (chippi_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, chippi_home):
        ...
        (chippi_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, chippi_home):
    ...
    def test_unknown_log_returns_none(self, chippi_home):
        from chippi_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, chippi_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from chippi_cli.debug import _capture_log_snapshot

        logs_dir = chippi_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, chippi_home):
    ...
    def test_falls_back_when_primary_empty(self, chippi_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of CHIPPI_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def chippi_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated CHIPPI_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".chippi"
        home.mkdir()
        monkeypatch.setenv("CHIPPI_HOME", str(home))
        ...
        monkeypatch.delenv("CHIPPI_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, chippi_home_with_secret):
        from chippi_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, chippi_home_with_secret):
        from chippi_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, chippi_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when CHIPPI_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("CHIPPI_REDACT_SECRETS", "false")

        from chippi_cli.debug import _capture_log_snapshot

        assert os.environ.get("CHIPPI_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, chippi_home_with_secret
    ):
        from chippi_cli.debug import _capture_log_snapshot

        log_path = chippi_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, chippi_home_with_secret):
        from chippi_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from chippi_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, chippi_home):
        from chippi_cli.debug import collect_debug_report

        with patch("chippi_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- chippi dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- chippi dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, chippi_home):
        from chippi_cli.debug import collect_debug_report

        with patch("chippi_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, chippi_home):
        from chippi_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, chippi_home):
        from chippi_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, chippi_home, capsys):
        ...
        with patch("chippi_cli.dump.run_dump"), \
             patch("chippi_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("chippi_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, chippi_home, capsys):
        ...
        with patch("chippi_cli.dump.run_dump"), \
             patch(
                 "chippi_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, chippi_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, chippi_home, capsys):
        ...
        with patch("chippi_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- chippi dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- chippi dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- chippi dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, chippi_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from chippi_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("chippi_cli.dump.run_dump"), \
             patch("chippi_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("chippi_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, chippi_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, chippi_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, chippi_home_with_secret, capsys
    ):
        ...
        with patch("chippi_cli.dump.run_dump"), \
             patch("chippi_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("chippi_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "chippi debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, chippi_home):
        from chippi_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from chippi_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from chippi_cli.debug import delete_paste
        ...
        with patch("chippi_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.chippi/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``chippi debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, chippi_home):
        ...
        import ast
        import inspect
        from chippi_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, chippi_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from chippi_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, chippi_home):
    ...
    def test_merges_with_existing_pending(self, chippi_home):
        """Subsequent calls merge into existing pending.json."""
        from chippi_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, chippi_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, chippi_home):
    ...
    def test_sweep_deletes_expired_entries(self, chippi_home):
        ...
        with patch("chippi_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, chippi_home):
        from chippi_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("chippi_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, chippi_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, chippi_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, chippi_home):
        from chippi_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("chippi_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, chippi_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: chippi debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from chippi_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("chippi_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("chippi_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, chippi_home, capsys):
        from chippi_cli.debug import run_debug_share
        ...
        with patch("chippi_cli.dump.run_dump"), \
             ...
             patch("chippi_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, chippi_home, capsys):
        from chippi_cli.debug import run_debug_share
        ...
        with patch("chippi_cli.dump.run_dump"), \
             patch("chippi_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("chippi_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, chippi_home, capsys):
        from chippi_cli.debug import run_debug_share
