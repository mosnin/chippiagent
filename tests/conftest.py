"""Shared fixtures for the chippi-agent test suite.
...
2. **Isolated CHIPPI_HOME.** CHIPPI_HOME points to a per-test tempdir so
   code reading ``~/.chippi/*`` via ``get_chippi_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".chippi"`` instead of the canonical
   ``get_chippi_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CHIPPI_SESSION_* inheritance** — the agent's current gateway
...
# CHIPPI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CHIPPI_BEHAVIORAL_VARS = frozenset({
    "CHIPPI_YOLO_MODE",
    "CHIPPI_INTERACTIVE",
    "CHIPPI_QUIET",
    ...
    "CHIPPI_SESSION_CHAT_ID",
    "CHIPPI_SESSION_CHAT_NAME",
    "CHIPPI_SESSION_THREAD_ID",
    "CHIPPI_SESSION_SOURCE",
    "CHIPPI_SESSION_KEY",
    "CHIPPI_GATEWAY_SESSION",
    "CHIPPI_PLATFORM",
    "CHIPPI_MODEL",
    "CHIPPI_INFERENCE_MODEL",
    "CHIPPI_INFERENCE_PROVIDER",
    "CHIPPI_TUI_PROVIDER",
    "CHIPPI_MANAGED",
    "CHIPPI_DEV",
    "CHIPPI_CONTAINER",
    ...
    "CHIPPI_TIMEZONE",
    "CHIPPI_REDACT_SECRETS",
    "CHIPPI_BACKGROUND_NOTIFICATIONS",
    "CHIPPI_EXEC_ASK",
    "CHIPPI_HOME_MODE",
    "CHIPPI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.chippi/kanban.db instead of the per-test CHIPPI_HOME.
    "CHIPPI_KANBAN_DB",
    "CHIPPI_KANBAN_BOARD",
    "CHIPPI_KANBAN_HOME",
    "CHIPPI_KANBAN_WORKSPACES_ROOT",
    "CHIPPI_KANBAN_LOGS_ROOT",
    "CHIPPI_KANBAN_TASK",
    "CHIPPI_KANBAN_WORKSPACE",
    "CHIPPI_KANBAN_RUN_ID",
    "CHIPPI_KANBAN_CLAIM_LOCK",
    "CHIPPI_KANBAN_DISPATCH_IN_GATEWAY",
    "CHIPPI_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CHIPPI_HOME to per-test tempdirs so code that
    reads ``~/.chippi/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CHIPPI_* vars that could change test semantics.
    for name in _CHIPPI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect CHIPPI_HOME to a per-test tempdir. Code that reads
    #    ``~/.chippi/*`` via ``get_chippi_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.chippi/*`` via
    #    ``Path.home() / ".chippi"`` instead of ``get_chippi_home()``
    #    is a bug to fix at the callsite.
    fake_chippi_home = tmp_path / "chippi_test"
    fake_chippi_home.mkdir()
    (fake_chippi_home / "sessions").mkdir()
    (fake_chippi_home / "cron").mkdir()
    (fake_chippi_home / "memories").mkdir()
    (fake_chippi_home / "skills").mkdir()
    monkeypatch.setenv("CHIPPI_HOME", str(fake_chippi_home))
    ...
    #    ~/.chippi/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import chippi_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_chippi_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal chippi config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``chippi-gateway`` process
...
# correlated with a ``tests/chippi_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> chippi-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart chippi-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting chippi/python patterns are also blocked.
    """
    ...
    def _matches_chippi_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CHIPPI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_chippi_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch chippi-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m chippi_cli.main".
                if (
                    "chippi" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live chippi-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting chippi/python could hit the live gateway. "
