...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    chippi_state = ModuleType("chippi_state")
    chippi_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "chippi_state", chippi_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Chippi"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Chippi"
