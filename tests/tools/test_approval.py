...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("chippi_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("chippi_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"CHIPPI_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_chippi_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.chippi/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_chippi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $CHIPPI_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_chippi_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$CHIPPI_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_chippi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $CHIPPI_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.chippi/chippi-agent && source venv/bin/activate && python -m chippi_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m chippi_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m chippi_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "chippi_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart chippi-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_chippi_detected(self):
        """pkill targeting chippi/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_chippi_detected(self):
        cmd = "killall chippi"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep chippi) bypasses the pkill/killall name-matching
    ...
    def test_pkill_chippi_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("CHIPPI_GATEWAY_SESSION", "CHIPPI_YOLO_MODE",
                      "CHIPPI_SESSION_KEY", "CHIPPI_INTERACTIVE")
        }
        os.environ.pop("CHIPPI_YOLO_MODE", None)
        os.environ.pop("CHIPPI_INTERACTIVE", None)
        os.environ["CHIPPI_GATEWAY_SESSION"] = "1"
        os.environ["CHIPPI_SESSION_KEY"] = self.SESSION_KEY
