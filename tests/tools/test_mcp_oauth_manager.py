...
def test_chippi_provider_subclass_exists():
    """ChippiMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CHIPPI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CHIPPI_PROVIDER_CLS is not None
    assert issubclass(_CHIPPI_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_chippi_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ChippiMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CHIPPI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CHIPPI_HOME", str(tmp_path))
    ...
    assert _CHIPPI_PROVIDER_CLS is not None
    assert isinstance(provider, _CHIPPI_PROVIDER_CLS)
    assert provider._chippi_server_name == "srv"
