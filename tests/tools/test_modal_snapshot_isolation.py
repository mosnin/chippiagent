...
def _restore_tool_modules():
    original_chippi_home = os.environ.get("CHIPPI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_chippi_home is None:
            os.environ.pop("CHIPPI_HOME", None)
        else:
            os.environ["CHIPPI_HOME"] = original_chippi_home
        _reset_modules(("tools", "chippi_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    chippi_cli = types.ModuleType("chippi_cli")
    chippi_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["chippi_cli"] = chippi_cli
    chippi_home = tmp_path / "chippi-home"
    os.environ["CHIPPI_HOME"] = str(chippi_home)
    sys.modules["chippi_cli.config"] = types.SimpleNamespace(
        get_chippi_home=lambda: chippi_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="chippi-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": chippi_home / "modal_snapshots.json",
