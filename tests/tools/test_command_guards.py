...
def _clean_state():
    ...
    saved = {}
    for k in ("CHIPPI_INTERACTIVE", "CHIPPI_GATEWAY_SESSION", "CHIPPI_EXEC_ASK", "CHIPPI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CHIPPI_INTERACTIVE"] = "1"
        session_key = os.getenv("CHIPPI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CHIPPI_INTERACTIVE or CHIPPI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
