...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._chippi_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_chippi_bin(self):
        """After _INSTALL_FAILED, manual install in CHIPPI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        chippi_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(chippi_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(chippi_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._chippi_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == chippi_bin
            assert _tirith_mod._resolved_path == chippi_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CHIPPI_HOME isolation
# ---------------------------------------------------------------------------

class TestChippiHomeIsolation:
    def test_chippi_bin_dir_respects_chippi_home(self):
        """_chippi_bin_dir must use CHIPPI_HOME, not hardcoded ~/.chippi."""
        from tools.tirith_security import _chippi_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CHIPPI_HOME": tmpdir}):
            result = _chippi_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_chippi_home(self):
        """_failure_marker_path must use CHIPPI_HOME, not hardcoded ~/.chippi."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CHIPPI_HOME": "/custom/chippi"}):
            result = _failure_marker_path()
        assert result == "/custom/chippi/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CHIPPI_HOME; verify it's active."""
        chippi_home = os.getenv("CHIPPI_HOME")
        assert chippi_home is not None, "CHIPPI_HOME should be set by conftest"
        assert "chippi_test" in chippi_home, "Should point to test temp dir"

    def test_get_chippi_home_fallback(self):
        """Without CHIPPI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_chippi_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CHIPPI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".chippi")
            result = _get_chippi_home()
