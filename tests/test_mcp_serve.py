"""
Tests for mcp_serve — Chippi MCP server.
...
def _isolate_chippi_home(tmp_path, monkeypatch):
    """Redirect CHIPPI_HOME to a temp directory."""
    monkeypatch.setenv("CHIPPI_HOME", str(tmp_path))
    try:
        import chippi_constants
        monkeypatch.setattr(chippi_constants, "get_chippi_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking chippi_state schema."""
