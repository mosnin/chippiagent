...
class TestToolsetConsistency:
    ...
    def test_chippi_platforms_share_core_tools(self):
        """All chippi-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on chippi-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["chippi-cli", "chippi-telegram", "chippi-discord", "chippi-whatsapp", "chippi-slack", "chippi-signal", "chippi-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_chippi_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("chippi-whatsapp")

    def test_chippi_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("chippi-api-server")
