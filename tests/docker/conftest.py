...
Override the image with ``CHIPPI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CHIPPI_TEST_IMAGE", "chippi-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CHIPPI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CHIPPI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged chippi user
# ---------------------------------------------------------------------------
#
# Background: every Chippi runtime path inside the container drops to UID
# 10000 (the ``chippi`` user) via ``s6-setuidgid chippi``. ``docker exec``
...
# fails (PermissionError swallowed) for chippi, so a test that ran as root
...
# helpers below run every probe under ``-u chippi`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: chippi).
