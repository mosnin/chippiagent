"""Tests for chippi_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Chippi entry point
(chippi, chippi-agent, chippi-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Chippi entry point imports chippi_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported chippi_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("chippi_bootstrap", None)
    import chippi_bootstrap  # noqa: WPS433
    return chippi_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Chippi CLI
...
class TestEntryPointsImportBootstrap:
    """Every Chippi entry point must import chippi_bootstrap as its
    ...
    # Entry points that invoke Chippi as a process.  Each one must
    # import chippi_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "chippi_cli/main.py",   # chippi CLI (console_script)
        "run_agent.py",          # chippi-agent (console_script)
        "acp_adapter/entry.py",  # chippi-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import chippi_bootstrap' and that
        ...
        half-finished ``chippi update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``chippi_bootstrap`` as a top-level module) leaves chippi
        ...
        """
        # Resolve relative to the chippi-agent repo root.  Tests live
        # at tests/test_chippi_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # chippi start even when chippi_bootstrap hasn't been
        ...
        assert first_import_name == "chippi_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'chippi_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import chippi_bootstrap' line to be the first import."
        )
