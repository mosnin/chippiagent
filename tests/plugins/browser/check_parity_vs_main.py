...
Run from the PR worktree:
    ...
    cd ~/.chippi/chippi-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.chippi/chippi-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.chippi/chippi-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a chippi-agent checkout"
)
...
# Isolated CHIPPI_HOME for the config write.
