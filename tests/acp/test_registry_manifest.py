"""Tests for ACP Registry metadata shipped with Chippi."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "chippi-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Chippi Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/chippi-agent"
    assert data["website"].startswith("https://chippi-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"chippi-agent[acp]=={data['version']}"
    assert uvx["args"] == ["chippi-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"chippi-agent[acp]=={_pyproject_version()}"
    )
