# Chippi Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: chippi-achievements plugin
...
### `GET /api/plugins/chippi-achievements/achievements`
...
### `POST /api/plugins/chippi-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/chippi-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/chippi-achievements/overview`
...
- Persist plugin data under:
  - `~/.chippi/plugins/chippi-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.chippi/plugins/chippi-achievements/scan_checkpoint.json`
...
- `~/.chippi/plugins/chippi-achievements/`
