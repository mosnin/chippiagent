...
Let the chippi agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where chippi runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `chippi meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `chippi meet` CLI |
...
| `cli.py` | `chippi meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CHIPPI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `chippi meet node {run,list,approve,remove,status,ping}` |
...
chippi meet install                                      # pip + Chromium
chippi meet setup                                        # preflight
chippi meet auth                                         # optional
chippi meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
chippi meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.chippi/.env
chippi meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
chippi meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
chippi meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.chippi/.env
chippi meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, chippi will **not** switch your system audio input automatically — the
...
chippi plugins enable google_meet
chippi meet node run --display-name my-mac --host 0.0.0.0 --port 18789
