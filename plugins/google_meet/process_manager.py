...
session-scoped state file under ``$CHIPPI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CHIPPI_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_chippi_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this chippi install, leave it first —
    ...
    env = os.environ.copy()
    env["CHIPPI_MEET_URL"] = url
    env["CHIPPI_MEET_OUT_DIR"] = str(out)
    env["CHIPPI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CHIPPI_MEET_HEADED"] = "1"
    if auth_state:
        env["CHIPPI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CHIPPI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CHIPPI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CHIPPI_MEET_MODE"] = mode
    if realtime_model:
        env["CHIPPI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CHIPPI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CHIPPI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CHIPPI_MEET_REALTIME_KEY"] = realtime_api_key
