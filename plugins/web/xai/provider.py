...
``{title, url, description, position}`` rows every other Chippi web
provider produces.
...
prefers Chippi-managed xAI Grok OAuth (via ``chippi auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.chippi/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``chippi tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `chippi auth` to sign in with "
