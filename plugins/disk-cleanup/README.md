...
Auto-tracks and cleans up ephemeral files created during Chippi Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CHIPPI_HOME` and `/tmp/chippi-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CHIPPI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CHIPPI_HOME` or `/tmp/chippi-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CHIPPI_HOME/disk-cleanup/` is itself excluded
- `$CHIPPI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
