"""disk_cleanup — ephemeral file cleanup for Chippi Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CHIPPI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CHIPPI_HOME and /tmp/chippi-*
Never touches: ~/.chippi/logs/ or any system directory.
"""
...
try:
    ...
    def get_chippi_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CHIPPI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".chippi").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CHIPPI_HOME/logs/``."""
    return get_chippi_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CHIPPI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CHIPPI_HOME or ``/tmp/chippi-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    chippi_home = get_chippi_home()
    try:
        path.resolve().relative_to(chippi_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/chippi-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("chippi-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CHIPPI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CHIPPI_HOME (but leave CHIPPI_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "chippi-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(chippi_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == chippi_home:
                continue
            try:
                rel_parts = dirpath.relative_to(chippi_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    chippi_home = get_chippi_home()
    try:
        rel = path.resolve().relative_to(chippi_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "chippi-agent",
        }:
            return None
        ...
        # Path isn't under CHIPPI_HOME (e.g. /tmp/chippi-*) — fall through.
        pass
