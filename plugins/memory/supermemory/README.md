...
Config file: `$CHIPPI_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `chippi` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `chippi-{identity}` → `chippi-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Chippi profile:

```json
{
  "container_tag": "chippi-{identity}"
}
```

For a profile named `coder`, this resolves to `chippi-coder`. The default profile resolves to `chippi-default`. Without `{identity}`, all profiles share the same container.
