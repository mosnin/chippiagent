...
def _load_supermemory_config(chippi_home: str) -> dict:
    config = _default_config()
    config_path = Path(chippi_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, chippi_home: str) -> None:
    config_path = Path(chippi_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `chippi memory setup`.
        # All other options are documented for $CHIPPI_HOME/supermemory.json
    ...
    def save_config(self, values, chippi_home):
        ...
        _save_supermemory_config(sanitized, chippi_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from chippi_constants import get_chippi_home
        self._chippi_home = kwargs.get("chippi_home") or str(get_chippi_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._chippi_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "chippi", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "chippi_memory", "target": target, "type": "explicit_memory"},
