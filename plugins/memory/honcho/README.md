...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/chippi>
...
```bash
chippi honcho setup    # full interactive wizard (cloud or local)
chippi memory setup    # generic picker, also works
```

Or manually:
```bash
chippi config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CHIPPI_HOME/honcho.json` | Profile-local (isolated Chippi instances) |
| 2 | `~/.chippi/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Chippi profile: `chippi` (default) or `chippi.<profile>`.
...
| 4 | `per-session` strategy | Chippi session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`chippi-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`chippi`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-chippi-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening chippi in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Chippi session ID (timestamp + hex). Every `chippi` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Chippi profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`chippi`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Chippi profile: `chippi` (default) or `chippi.<profile>` (e.g. `chippi -p coder` → host key `chippi.coder`).
...
| `CHIPPI_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `chippi honcho setup` | Full interactive setup wizard |
| `chippi honcho status` | Show resolved config for active profile |
| `chippi honcho enable` / `disable` | Toggle Honcho for active profile |
| `chippi honcho mode <mode>` | Change recall or observation mode |
| `chippi honcho peer --user <name>` | Update user peer name |
| `chippi honcho peer --ai <name>` | Update AI peer name |
| `chippi honcho tokens --context <N>` | Set context token budget |
| `chippi honcho tokens --dialectic <N>` | Set dialectic max chars |
| `chippi honcho map <name>` | Map current directory to a session name |
| `chippi honcho sync` | Create host blocks for all Chippi profiles |
