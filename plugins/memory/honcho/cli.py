...
Handles: chippi honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "chippi" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Chippi profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'chippi honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'chippi honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `chippi update` -- no output, no exceptions.
    """
    try:
        from chippi_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Chippi profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CHIPPI_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_chippi_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Chippi persistent cross-session memory.")
    ...
    chippi_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'chippi honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = chippi_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        chippi_host["peerName"] = new_peer

    current_ai = chippi_host.get("aiPeer") or cfg.get("aiPeer", "chippi")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        chippi_host["aiPeer"] = new_ai

    current_workspace = chippi_host.get("workspace") or cfg.get("workspace", "chippi")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        chippi_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = chippi_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        chippi_host["observationMode"] = new_obs
    else:
        chippi_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(chippi_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        chippi_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        chippi_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = chippi_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        chippi_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = chippi_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        chippi_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                chippi_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(chippi_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            chippi_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        chippi_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        chippi_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        chippi_host["dialecticReasoningLevel"] = new_reasoning
    else:
        chippi_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = chippi_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        chippi_host["sessionStrategy"] = new_strat

    chippi_host["enabled"] = True
    chippi_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        chippi_config = load_config()
        chippi_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(chippi_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: chippi config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    chippi honcho status     -- show full config")
    print("    chippi honcho mode       -- change recall/observation mode")
    print("    chippi honcho tokens     -- tune context and dialectic budgets")
    print("    chippi honcho peer       -- update peer names")
    print("    chippi honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Chippi profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: chippi honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'chippi honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: chippi honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        chippi = hosts.get(_host_key(), {})
        user = chippi.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = chippi.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = chippi.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = chippi.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Chippi' identity in Honcho. Seed with 'chippi honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: chippi honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: chippi honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = chippi.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = chippi.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = chippi.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Chippi asks Honcho's AI peer a question")
        ...
        print("\n  Set with: chippi honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'chippi honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'chippi honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    chippi honcho identity --show        — show both peer representations")
        print("    chippi honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Chippi + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Chippi\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Chippi persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  chippi honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'chippi honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'chippi honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  chippi honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'chippi'")
        print("  with Honcho configured and no prior session history.")
        print("  (Chippi calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    chippi honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'chippi honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Chippi, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'chippi honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    chippi honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  chippi honcho identity <path/to/SOUL.md>")
    ...
    print("    Chippi:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Chippi:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Chippi:   Honcho observes every message and updates representations")
    ...
    print("    Chippi:   per-session by default — each run gets its own session")
    print("              Map a custom name:  chippi honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. chippi honcho setup              — configure API key (required)")
        print("  2. chippi honcho migrate            — re-run this walkthrough")
    else:
        print("  1. chippi honcho status             — verify Honcho connection")
        print("  2. chippi                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. chippi honcho identity --show    — verify AI peer representation")
        print("  4. chippi honcho tokens             — tune context and dialectic budgets")
        print("  5. chippi honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'chippi memory setup'...\n")
        from chippi_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``chippi honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``chippi honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to chippi memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Chippi Honcho",
    )
