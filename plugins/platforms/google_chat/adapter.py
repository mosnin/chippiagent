...
# ``model_tools`` import time, so ``chippi status``, ``chippi chat``, etc.
...
# ``chippi_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Chippi is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a chippi MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CHIPPI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _chippi_home = _get_chippi_home()
        except (ModuleNotFoundError, ImportError):
            _chippi_home = _Path.home() / ".chippi"
        self._thread_count_store = _ThreadCountStore(
            _chippi_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Chippi is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CHIPPI_HOME", str(_Path.home() / ".chippi"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CHIPPI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a chippi MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Chippi is thinking…' marker message.
        ...
        "Chippi is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Chippi is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Chippi is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Chippi is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Chippi is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``chippi setup``.

    The setup wizard at ``chippi_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.chippi/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. chippi-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.chippi/.env")
    print_info("Restart the gateway: chippi gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``chippi cron`` running as a
    separate process from ``chippi gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Chippi plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'chippi-agent[google_chat]'",
        ...
        # the "Chippi is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Chippi is thinking…' marker message "
