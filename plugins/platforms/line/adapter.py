"""
LINE Messaging API platform adapter for Chippi Agent.
...
to Chippi Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CHIPPI_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            chippi_home = Path.home().joinpath(".chippi").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``chippi status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``chippi status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``chippi setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.chippi/.env`` via ``chippi_cli.config``.
    """
    print()
    ...
    try:
        from chippi_cli.config import get_env_var, set_env_var
    except ImportError:
        print("chippi_cli.config not available; set LINE_* vars manually in ~/.chippi/.env")
        return
