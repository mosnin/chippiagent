"""
Microsoft Teams platform adapter for Chippi Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Chippi"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        chippi_action = data.get("chippi_action", "")
        session_key = data.get("session_key", "")

        if not chippi_action or not session_key:
        ...
        choice = choice_map.get(chippi_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="chippi_approve",
                    data={**btn_data_base, "chippi_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="chippi_approve",
                    data={**btn_data_base, "chippi_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="chippi_approve",
                    data={**btn_data_base, "chippi_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="chippi_approve",
                    data={**btn_data_base, "chippi_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Chippi\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.chippi/.env")
