"""SimpleX Chat platform adapter (Chippi plugin).
...
This adapter ships as a Chippi platform plugin under
``plugins/platforms/simplex/``. The Chippi plugin loader scans the
...
discoverable and `chippi setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Chippi — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"chippi-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``chippi setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.chippi/.env`` via ``chippi_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("chippi_cli.config not available; set SIMPLEX_* vars manually in ~/.chippi/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `chippi gateway status` without
