...
_CHIPPI_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CHIPPI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `chippi tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CHIPPI_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Chippi toolsets (CLI + messaging platforms)
    #
    ...
    "chippi-cli": {
        ...
        "tools": _CHIPPI_CORE_TOOLS,
        "includes": []
    },

    "chippi-cron": {
        # Mirrors chippi-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `chippi tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as chippi-cli; gated by `chippi tools`",
        "tools": _CHIPPI_CORE_TOOLS,
        "includes": []
    },
    ...
    "chippi-email": {
        "description": "Email bot toolset - interact with Chippi via email (IMAP/SMTP)",
        "tools": _CHIPPI_CORE_TOOLS,
        "includes": []
    },
    ...
    "chippi-sms": {
        "description": "SMS bot toolset - interact with Chippi via SMS (Twilio)",
        "tools": _CHIPPI_CORE_TOOLS,
        "includes": []
    },
    ...
    "chippi-webhook": {
        ...
        "tools": _CHIPPI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "chippi-gateway": {
        ...
        "tools": [],
        "includes": ["chippi-telegram", "chippi-discord", "chippi-whatsapp", "chippi-slack", "chippi-signal", "chippi-bluebubbles", "chippi-homeassistant", "chippi-email", "chippi-sms", "chippi-mattermost", "chippi-matrix", "chippi-dingtalk", "chippi-feishu", "chippi-wecom", "chippi-wecom-callback", "chippi-weixin", "chippi-qqbot", "chippi-webhook", "chippi-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (chippi-<name>).
        # Gives them _CHIPPI_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("chippi-"):
            platform_name = name[len("chippi-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CHIPPI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
